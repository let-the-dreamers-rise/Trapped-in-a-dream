...
(function(){ var t = window.GATE_DATA.questions['apti'].topics.find(function(t){return t.id==='apti-quant';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n\nPercentage. A percentage is just a fraction whose denominator has been forced to 100 — \"20%\" literally means 20 out of every 100, nothing more mysterious. To convert any fraction to a percentage, multiply it by 100; to convert a percentage back to a fraction, divide by 100 and simplify.\n\nKEY: Fraction-percentage table you must know cold — 1/2=50%, 1/3=33.33%, 1/4=25%, 1/5=20%, 1/6=16.67%, 1/8=12.5%, 1/9=11.11%, 1/10=10%, 1/11=9.09%, 1/12=8.33%, 2/3=66.67%, 3/4=75%. Recognising these instantly turns a 45-second calculation into a 5-second lookup.\n\nRatio and proportion. A ratio a:b compares two quantities by division, not subtraction — it tells you how many times one quantity fits into the other, and the actual values could be anything as long as they hold that relationship. A proportion is simply a statement that two ratios are equal, written a:b = c:d, and it lets you find an unknown by cross-multiplying: a×d = b×c.\n\na/b = c/d  ⇒  a×d = b×c\n\nPer unit reasoning. \"Per unit\" means dividing a total by however many units produced it, giving you the rate for exactly one item — one hour, one worker, one kilometre. Once you have the per-unit value, you scale it up by multiplying by however many units the question actually asks about; this single idea underlies work-rate, speed, and pricing problems.\n\nPer-unit reasoning shows up everywhere once you know to look for it:\n• Speed. distance per unit of time — divide distance by time to get the per-unit (per hour/second) rate.\n• Work rate. work done per unit of time by one worker — the reciprocal of the time that worker alone needs to finish the job.\n• Unit price. cost per single item — divide total cost by quantity before comparing two differently-sized packs.\n\nWhat an average really is. An average (arithmetic mean) is the value each item would hold if the total were redistributed perfectly equally among all items — it is Total ÷ Count, nothing more. It is NOT the average of the individual \"rates\" unless every item contributes an equal share to that total, which is the seed of one of the biggest traps below.\n\naverage = (sum of all values) / (number of values)\n\nSetting up unknowns from a word problem. The single most useful habit in quant is to name the unknown explicitly (let the unknown quantity be x) before writing anything else, then translate every sentence in the problem into one algebraic equation involving x. Read the question twice: once to identify what is actually being asked, and once to extract every numeric relationship, because the answer variable is often not the first quantity mentioned.\n\n1. Underline every number and every relationship word (more than, less than, times, of, ratio) in the question.\n2. Assign a variable to the smallest or most basic unknown quantity, not necessarily the one the question asks for.\n3. Translate each sentence into one equation; \"of\" almost always means multiply and \"is\" almost always means equals.\n4. Solve, then re-read the question to confirm you are reporting the exact quantity asked for, not an intermediate variable.\n\nKEY: To convert km/h to m/s multiply by 5/18; to convert m/s to km/h multiply by 18/5 — this single conversion pair appears in nearly every speed-time-distance question and is worth memorising rather than deriving each time.\n\nKEY: When a word problem gives only a ratio plus one actual total, first find the common multiplier k by dividing the total by the sum of the ratio's parts, then multiply every ratio term by k to recover the real values.\n\nGATE TRAP: Solving beautifully for x and then submitting x as the final answer when the question actually asked for 2x, or for x+5, or for \"the other quantity\" — always re-map your solved variable back onto the actual question before locking an answer.\n\nEVERY EDGE CASE\n\nGATE TRAP: Successive percentage changes are NEVER simply added or subtracted. A 20% increase followed by a 20% decrease does not return you to the original value — it leaves you at 96% of the original, a net loss of 4%, because the second change acts on a different (larger) base than the first.\n\nnet change for successive a% then b% = a + b + ab/100\n\nREMEMBER: The successive-change formula above works for any sequence of increases/decreases as long as you feed in negative values for decreases. A 10% increase then a 10% decrease gives 10 + (-10) + (10)(-10)/100 = -1%, a net 1% loss, not zero — this exact scenario appears on GATE almost every year.\n\nGATE TRAP: Average of averages is only valid when every group being averaged has the EXACT same number of items (equal weights). If class A (30 students) averages 60 marks and class B (20 students) averages 80 marks, the overall average is NOT (60+80)/2 = 70; it is the weighted average (30×60 + 20×80)/50 = 68, because class A's larger size pulls the true average down toward its own score.\n\nweighted average = (sum of value × weight) / (sum of weights)\n\nGATE TRAP: Ratio only tells you a relationship, never actual values. If ages are in ratio 3:5, the ages could be 3 and 5, or 30 and 50, or 300 and 500 — you can only pin down actual values once one more piece of real information (an actual age, a sum, a difference) is given, which fixes the common multiplier k.\n\nGATE TRAP: Inclusive vs exclusive counting (the fence-post error). \"How many integers from 10 to 20 inclusive\" is 20 - 10 + 1 = 11 numbers, not 10 — you must add 1 when both endpoints are included. Fence posts along a straight 100 m fence with posts every 10 m need 11 posts, not 10, because a post also stands at each end.\n\ncount of integers from a to b inclusive = b - a + 1\n\nGATE TRAP: Relative speed depends entirely on direction. When two bodies move in the SAME direction, relative speed is the difference of their speeds (the faster one gains on the slower one at that reduced rate); when they move in OPPOSITE directions (or toward each other), relative speed is the SUM of their speeds. Mixing these up is the single most common speed-time-distance error on GATE.\n\nsame direction: relative speed = v1 - v2\n\nopposite direction: relative speed = v1 + v2\n\nREMEMBER: Upstream and downstream reverse the roles of speed addition and subtraction relative to still water. Downstream speed = boat speed + current speed (current helps you), and upstream speed = boat speed - current speed (current fights you); from these two, boat speed = (downstream + upstream)/2 and current speed = (downstream - upstream)/2.\n\nboat speed = (down + up) / 2\n\ncurrent speed = (down - up) / 2\n\nGATE TRAP: Work-rate problems change completely when a worker leaves midway or joins late. You cannot use the simple combined-rate formula for the whole job; instead, add up the fraction of work each worker actually completed during the time they were present, and the unknowns must satisfy total fraction completed = 1 (the whole job), which usually means splitting the timeline into distinct phases.\n\nGATE TRAP: \"At least one\" probability problems are solved fastest — and most reliably — through the complement, because directly summing \"exactly one,\" \"exactly two,\" and so on invites double-counting or missed cases. P(at least one event happens) = 1 - P(none of the events happen), and P(none happen) is usually a straightforward product for independent events.\n\nP(at least one) = 1 - P(none)\n\nKEY: Mutually exclusive and independent are NOT the same idea and are often deliberately confused in options. Mutually exclusive events cannot both happen in a single trial (P(A and B) = 0), so they are actually strongly DEPENDENT on each other (if A happens, B definitely did not). Independent events have no influence on each other's probability at all (P(A and B) = P(A)×P(B)), and two events with nonzero probability can never be both mutually exclusive and independent at the same time.\n\nKEY: Percentage-point change and percent change measure completely different things and swapping them is a guaranteed wrong answer. If a rate moves from 20% to 25%, that is a rise of 5 percentage points, but a 25% increase in the rate itself (5/20 × 100). Always check exactly which phrase the question uses.\n\nWORKED EXAMPLES\n\nExample 1 — Successive percentage change. The price of an item is increased by 25% and then decreased by 20%. Find the net percentage change.\n\n1. Slow algebraic route: let the original price be 100. After a 25% increase, price = 100 × 1.25 = 125. After a 20% decrease on this NEW price, price = 125 × 0.80 = 100.\n2. Net change = 100 - 100 = 0, so there is no net change at all — the price returns exactly to its original value.\n3. Fast exam route: apply net change = a + b + ab/100 with a = 25, b = -20 directly: 25 + (-20) + (25)(-20)/100 = 5 - 5 = 0.\n4. Time saved: the slow route needs two multiplications and a subtraction (roughly 40 seconds); the formula gives the answer in one line (roughly 10 seconds) — a saving of about 30 seconds, which matters across 15 marks of aptitude questions under time pressure.\n\nExample 2 — Weighted average. A shop sold 40 units of item X at a profit of 10% and 60 units of item Y at a profit of 20%, with X and Y having the same cost price per unit. Find the overall percentage profit.\n\n1. Slow algebraic route: let cost price per unit = 100 for both. Total cost = 100×(40+60) = 10000. Profit from X = 40×100×0.10 = 400. Profit from Y = 60×100×0.20 = 1200. Total profit = 1600. Overall percentage profit = 1600/10000 × 100 = 16%.\n2. Fast exam route: since the \"weights\" here are the equal cost prices per unit, use the weighted average formula directly on the quantities: (40×10 + 60×20)/(40+60) = (400+1200)/100 = 16%.\n3. Time saved: the fast route skips assuming a cost-price value and multiplying it through twice, saving roughly 25-30 seconds versus the full numeric build-up.\n4. GATE TRAP check: a student who wrongly computes the simple average (10+20)/2 = 15% has fallen for the average-of-averages trap, since the two groups (40 units and 60 units) are unequal in size.\n\nExample 3 — Relative speed, opposite directions. Two trains, 120 m and 180 m long, run on parallel tracks in opposite directions at 54 km/h and 36 km/h. Find the time taken for them to completely cross each other.\n\n1. Slow algebraic route: convert speeds to m/s: 54 km/h = 15 m/s, 36 km/h = 10 m/s. Since they move in opposite directions, relative speed = 15 + 10 = 25 m/s. Total distance to be covered for complete crossing = sum of both lengths = 120 + 180 = 300 m. Time = distance/speed = 300/25 = 12 seconds.\n2. Fast exam route: memorise km/h to m/s conversion as ×5/18, so 54×5/18=15 and 36×5/18=10 in one step each; add lengths and speeds directly and divide — the arithmetic is identical but done without writing intermediate reasoning sentences.\n3. Time saved: writing out the reasoning in full sentences costs 20-30 seconds; jumping straight to numbers with the memorised conversion factor and the \"sum of lengths over sum of speeds\" template saves that time entirely.\n4. GATE TRAP check: using DIFFERENCE of speeds here (a same-direction rule) instead of SUM would give a wrong time of 300/5 = 60 seconds — always confirm direction before picking sum or difference.\n\nExample 4 — At least one probability via complement. Two dice are thrown together. Find the probability of getting at least one six.\n\n1. Slow algebraic route: total outcomes = 6×6 = 36. Count outcomes with at least one six directly: (6, x) for x=1..6 gives 6 outcomes, (x, 6) for x=1..6 gives 6 outcomes, but (6,6) is counted twice, so total = 6+6-1 = 11. Probability = 11/36.\n2. Fast exam route: use the complement. P(no six on a single die) = 5/6, so P(no six on either die) = 5/6 × 5/6 = 25/36. P(at least one six) = 1 - 25/36 = 11/36.\n3. Time saved: the direct-counting route risks the double-counting error at (6,6) and needs careful listing; the complement route is two clean multiplications and one subtraction, saving 20-25 seconds and removing the double-counting risk entirely.\n4. GATE TRAP check: forgetting to subtract the double-counted (6,6) case in the direct method is the classic slip that the complement method sidesteps automatically."; })();


(function(){ var t = window.GATE_DATA.questions['apti'].topics.find(function(t){return t.id==='apti-logical';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n\nValid vs merely persuasive. An argument is logically valid only when its conclusion is FORCED by its premises — true in absolutely every situation consistent with those premises — not merely when the conclusion sounds reasonable or is probably true in real life. GATE logical reasoning rewards ruthless literal-mindedness: you must judge only what the given statements guarantee, ignoring outside knowledge, common sense about the real world, or what \"usually\" happens.\n\nKEY: The test for validity is always the same — try to imagine ANY scenario, however unlikely, where every premise is true but the proposed conclusion is false. If you can imagine even one such scenario, the conclusion is not valid, no matter how sensible it looks.\n\nSetting up unknowns for puzzles. Arrangement, blood-relation, and coding-decoding puzzles are word problems in disguise — the fix is to externalise every fact onto paper immediately rather than tracking it in your head. Draw a row of blanks for linear arrangements, a circle of dots for circular arrangements, or a family tree with generation levels for blood relations, and fill in facts as you read them rather than after finishing the whole passage.\n\n1. Read the entire passage once without writing anything, to get the overall shape of the puzzle.\n2. Draw the skeleton structure (boxes in a row, a circle, a grid, or a tree) before placing a single fact.\n3. Place the most restrictive/definite clues first (an exact position, an exact relation) — vague clues like \"somewhere before\" go in last.\n4. After every placement, scan remaining clues to see if any is now forced by elimination, and repeat until no clue is left unused.\n\nReading a chart or diagram inside a logical puzzle. Many logical questions embed a small diagram — a network of relations, a Venn diagram, or a simple flowchart — and the single most common failure is reading only the labels without checking direction of arrows or which region a boundary actually encloses. Always trace boundaries and arrowheads explicitly with a finger or pencil-in-the-mind before answering, rather than eyeballing the shape.\n\nWhat makes a coding-decoding rule \"consistent\". A coding scheme maps each letter to another letter or symbol using one fixed rule (a shift, a reversal, a pairing) applied identically to every letter in the word — if a rule explains some letters but not others, it is the wrong rule, and you must test on ALL given letters before applying it to the unknown word.\n\nshifted letter position = (original position + shift) mod 26\n\nKEY: To decode a message instead of encoding one, apply the shift in reverse (subtract instead of add) — encoding and decoding rules for the same cipher differ only in the sign of the shift.\n\nKEY: For any arrangement puzzle, place the most restrictive clue (an exact seat number, an exact neighbour) first and the vaguest clue (\"somewhere to the left of\") last — solving in this order avoids having to backtrack and redraw the whole diagram.\n\nEVERY EDGE CASE\n\nGATE TRAP: In syllogisms, \"Some A are B\" NEVER implies \"Some A are not B\" — the \"some\" statement is completely silent about the rest of A; it is entirely possible that ALL A are B and the statement \"some A are B\" is still technically true (since \"some\" only requires at least one, not \"some but not all\").\n\nGATE TRAP: Correlation is not causation, even inside a tightly-worded logical argument. If a passage states two things increased together, that alone never proves one caused the other — a third factor could cause both, or the timing could be coincidental, or the causation could run in the reverse direction from what seems natural.\n\nGATE TRAP: Circular arrangement facing-direction ambiguity. When people sit around a circular table, \"facing the centre\" and \"facing away from the centre\" produce mirror-image (left-right reversed) seating orders for the exact same clues — always fix and re-confirm the facing direction stated in the question before deciding who is to someone's \"left\" or \"right\", because left and right swap between the two cases.\n\nGATE TRAP: Linear arrangement \"left\" and \"right\" depend on whose perspective is meant. \"A is to the left of B\" almost always means from an outside observer's viewpoint reading the row left-to-right, but if the question says \"as B sees it\" or the row represents people facing the reader, left and right can flip — always re-read for a stated viewpoint before assuming the default.\n\nGATE TRAP: Blood relation puzzles hide gender ambiguity in words like \"child,\" \"cousin,\" \"sibling,\" and \"parent's sibling's child.\" Never assume a gender unless the passage uses a gendered pronoun or noun (son, daughter, brother, sister) — an ungendered relation term must be tracked as unknown-gender until the passage confirms it, since assuming wrongly cascades into every later deduction.\n\nGATE TRAP: Coding-decoding letter shifts must wrap around correctly at the ends of the alphabet. Shifting Z forward by 3 does not run off the alphabet — it wraps to C, because position 26 + 3 = 29, and 29 mod 26 = 3, which is the 3rd letter, C. Forgetting the wraparound (mod 26) is the single most common numeric slip in shift-cipher questions.\n\nGATE TRAP: Inclusive vs exclusive counting also traps arrangement puzzles. \"How many people are seated between A and B\" going one way around a circular table of n people is a different count than going the other way, and the question's wording (\"between,\" \"from A to B\") decides whether the two endpoints A and B themselves are included in that count — always re-read exactly which count is being asked.\n\nKEY: In a number/letter series, always test BOTH an arithmetic (constant difference) pattern and a pattern of alternating or second-order differences before giving up — many GATE series are actually two interleaved simpler series (odd positions follow one rule, even positions follow another).\n\nGATE TRAP: \"Either...or\" statements in logical reasoning are typically treated as inclusive unless the passage states \"but not both\" — do not assume exclusivity is intended unless the question's wording makes it explicit, since assuming exclusivity injects a constraint the puzzle never actually gave you.\n\nKEY: When a puzzle's clues run out before every position is pinned down, look for a clue that ELIMINATES options rather than one that places directly — elimination clues (\"X is not at either end\") are easy to skim past but often the only way to break a tie between two otherwise-valid arrangements.\n\nREMEMBER: A single logical puzzle passage often supports more than one valid final arrangement unless you check every clue against your finished diagram one last time — always re-verify all clues against the completed solution before selecting an answer, since an early clue can silently stop holding after later placements.\n\nWORKED EXAMPLES\n\nExample 1 — Syllogism chaining. Statements: All doctors are readers. All readers are thinkers. Conclusion: All doctors are thinkers. Is the conclusion valid?\n\n1. Slow route: draw circle D (doctors) fully inside circle R (readers), since \"All doctors are readers.\" Draw circle R fully inside circle T (thinkers), since \"All readers are thinkers.\" Since D is inside R and R is inside T, D must be entirely inside T as well — there is no way to draw D outside T while keeping both premises true.\n2. Fast route: recognise the \"All A are B, All B are C gives All A are C\" chain pattern instantly — two universal statements sharing a middle term (readers) always chain when the shared term is the predicate of the first and subject of the second.\n3. Conclusion: valid.\n4. Time saved: drawing full Venn circles for every syllogism costs 30-40 seconds; recognising the standard chain pattern by name gives the same verified answer in under 10 seconds, a saving that compounds heavily since syllogisms often come in sets of 2-3 statements per question.\n\nExample 2 — Invalid syllogism trap. Statements: All cats are animals. All dogs are animals. Conclusion: Some cats are dogs. Is the conclusion valid?\n\n1. Slow route: draw circle Cats fully inside circle Animals, and circle Dogs fully inside circle Animals, but the two premises never force Cats and Dogs to touch — draw them as two completely disjoint circles both sitting inside Animals, and both original premises remain fully true while the conclusion \"Some cats are dogs\" is false in that diagram.\n2. Fast route: recognise the \"All A are B, All C are B gives no conclusion between A and C\" trap pattern immediately — two universal statements sharing the same predicate (animals) never chain, regardless of how natural the conclusion sounds.\n3. Conclusion: not valid (no relation between A and C can be concluded).\n4. Time saved: a student without the pattern memorised might spend a full minute trying different Venn arrangements before realising no valid link exists; recognising the shared-predicate pattern by name rejects the conclusion in seconds.\n\nExample 3 — Circular arrangement with facing direction. Six people sit around a circular table facing the centre. A is second to the right of B. C is second to the left of B. Find the position of A relative to C.\n\n1. Slow route: place B at a reference position (say, position 0 of a 6-seat circle numbered clockwise 0-5). \"Second to the right of B\" for a centre-facing arrangement means moving 2 seats clockwise, so A is at position 2. \"Second to the left of B\" means moving 2 seats anticlockwise, so C is at position 4 (i.e., -2 mod 6 = 4). The gap from A (2) to C (4) is 2 seats clockwise, so A is two seats to the left of C (equivalently, C is two seats to the right of A).\n2. Fast route: for people facing the centre, right = clockwise and left = anticlockwise (this is the fixed convention once facing direction is set) — assign B = 0, move +2 for \"right\", -2 for \"left\" directly using modular arithmetic on the seat count, without redrawing the whole circle.\n3. Time saved: sketching a fresh circle and physically walking around it for every clue costs 20-30 seconds per clue; using the +/- modular convention once the facing direction is fixed lets you compute positions numerically in seconds, especially valuable when a puzzle has 5-6 clues to place.\n4. GATE TRAP check: if the people instead faced AWAY from the centre, right and left would swap (right = anticlockwise), so always confirm facing direction is centre-facing before applying the +2/-2 convention as done here.\n\nExample 4 — Coding-decoding with wraparound. In a certain code, each letter is replaced by the letter 4 positions after it in the alphabet (A becomes E, B becomes F). What does \"WXYZ\" become in this code?\n\n1. Slow route: W is the 23rd letter; 23+4 = 27, which exceeds 26, so wrap: 27-26 = 1, giving A. X is 24th; 24+4=28, wrap to 28-26=2, giving B. Y is 25th; 25+4=29, wrap to 3, giving C. Z is 26th; 26+4=30, wrap to 4, giving D. So \"WXYZ\" becomes \"ABCD\".\n2. Fast route: apply shifted position = (original + shift) mod 26 directly, using mod-26 arithmetic in one step per letter rather than a separate \"check if it exceeds 26\" comparison: (23+4) mod 26 = 1, (24+4) mod 26 = 2, (25+4) mod 26=3, (26+4) mod 26 = 4 (since 26 mod 26 = 0, treat Z as position 0 for the modulus, or equivalently treat the result 0 as the 26th letter Z's shift target — using 1-indexed mod carefully). Recognising this pattern (last 4 letters of the alphabet map to the first 4) also gives an instant sanity check.\n3. Time saved: writing out the full alphabet and manually counting forward 4 letters for each letter (a common but slow method) costs 45-60 seconds for a 4-letter word with wraparound; the mod-26 formula applied directly gives the same answer in 15-20 seconds and generalises instantly to any shift or word length.\n4. GATE TRAP check: forgetting the wraparound and simply writing \"position 27, 28, 29, 30\" as if the alphabet kept extending past Z is the single most common error here — always reduce any position greater than 26 using mod 26."; })();


(function(){ var t = window.GATE_DATA.questions['apti'].topics.find(function(t){return t.id==='apti-verbal';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n\nWhat makes an argument valid vs merely persuasive. In critical reasoning, an argument is a claim (the conclusion) backed by reasons (the premises); it is valid only if the premises, taken as true, actually force the conclusion, not merely make it sound convincing or emotionally appealing. GATE verbal-reasoning questions almost always test whether you can spot a gap between \"this sounds right\" and \"this is actually proven,\" so train yourself to ask \"does this really have to follow, or does it just feel like it follows?\"\n\nKEY: Assumption, inference, and conclusion are three different things and GATE deliberately mixes them up in answer options. An assumption is an UNSTATED premise the argument needs to work; an inference is something you can validly DERIVE from what is stated; a conclusion is the main point the argument is trying to prove. Confusing \"what must be assumed\" with \"what can be concluded\" is the single most common verbal-reasoning error.\n\nReading comprehension basics. A comprehension passage is built from a main idea (what the whole passage is about), supporting details (facts, examples, data used to back up the main idea), and often an author's tone or opinion layered on top of the facts. Before reading the questions, skim the passage once for structure — where does the argument turn, where does it introduce a contrast, where does it conclude — rather than trying to memorise every fact on a single pass.\n\n1. Read the passage once for overall structure and tone, not for detail.\n2. Read each question and identify exactly what type it is: main-idea, detail, inference, vocabulary-in-context, or tone.\n3. For detail and vocabulary questions, return to the exact line in the passage rather than relying on memory.\n4. For main-idea and inference questions, check that your chosen option is supported by the WHOLE passage, not just one line.\n\nVocabulary and word relationships. GATE verbal tests direct meaning (synonyms, antonyms), word usage in context, and analogies (word pairs sharing the same underlying relationship, such as \"doctor is to hospital as teacher is to school\"). Building vocabulary from zero means learning words in groups by their relationship type — synonym clusters, prefix/suffix families (e.g., \"in-\" and \"un-\" both often mean \"not\") — rather than as an unconnected list.\n\nSentence construction and grammar from scratch. A grammatically correct English sentence needs a subject and a verb that agree in number (a singular subject takes a singular verb, a plural subject takes a plural verb), and every pronoun must clearly refer back to exactly one noun (its antecedent) without ambiguity. Reading a sentence aloud in your head, slowly, is often enough to catch an agreement error that skimming misses.\n\nsubject (singular/plural) + matching verb form = grammatically correct clause\n\nKEY: For vocabulary analogies, name the exact relationship in words before matching an answer — \"doctor : hospital\" is a \"worker : workplace\" relationship, and the correct option must share that SAME relationship, not just a loose thematic similarity like \"doctor : patient.\"\n\nKEY: For \"fill in the blank\" vocabulary questions, first decide whether the missing word should be positive, negative, or neutral in tone from the sentence's other clues, then eliminate every option with the wrong tone before comparing precise shades of meaning among what remains.\n\nKEY: When a comprehension question asks for the passage's main idea, the correct option must cover the ENTIRE passage, not just its most memorable paragraph — an option describing only one supporting example, however vividly worded, is a trap for main-idea questions.\n\nREMEMBER: In critical reasoning, \"strengthen the argument\" and \"weaken the argument\" questions both work on the argument's weakest link (its central assumption) — find that assumption first, then look for the option that most directly confirms it (to strengthen) or most directly breaks it (to weaken).\n\nKEY: A pronoun (it, they, this, which) must refer unambiguously to exactly one noun stated earlier in the sentence — if a sentence has two nouns either of which the pronoun could plausibly mean, the sentence is genuinely ambiguous and any option relying on one specific reading is suspect.\n\nEVERY EDGE CASE\n\nGATE TRAP: Correlation is not causation. A passage stating that two things rose or fell together (ice cream sales and drowning incidents both rise in summer) never proves one caused the other — a hidden third factor (hot weather driving both) or pure coincidence can produce the same pattern, and a valid inference must never assume causation the passage does not explicitly establish.\n\nGATE TRAP: \"Some\" statements never imply \"some are not.\" If a passage says \"some students in the class passed,\" this does NOT let you infer that some students failed — it is entirely consistent with every single student having passed, since \"some\" only guarantees at least one, not \"not all.\"\n\nGATE TRAP: An inference must be provable strictly from the passage, not merely consistent with it or \"probably true\" given general knowledge. If a passage never states the age of a character, \"the character is young\" is not a valid inference even if every other clue suggests youth — GATE inference questions are looking for the option that MUST be true, not the option that seems most likely.\n\nGATE TRAP: Do not confuse an assumption with a conclusion when identifying \"which of the following is assumed by the argument.\" An assumption is invisible in the text — you must ask \"what does the argument silently need to be true for its stated reasoning to work?\" — whereas a conclusion is the argument's own explicitly stated final claim.\n\nGATE TRAP: Para-jumble transition words are the strongest clue, and skipping them is the most common way to lose these marks. Words like \"however,\" \"therefore,\" \"this,\" and \"such\" refer back to something in a PRECEDING sentence, so a sentence starting with \"However\" or \"This effect\" can never be the first sentence of the jumbled paragraph — use these anchor words to lock in relative order before trying to guess the whole sequence at once.\n\nGATE TRAP: Confusable word pairs are deliberately tested — \"affect\" (verb, to influence) vs \"effect\" (noun, a result); \"principal\" (main, or head of a school) vs \"principle\" (a fundamental rule); \"compliment\" (praise) vs \"complement\" (something that completes). Learn these in pairs with a memory hook rather than in isolation, since GATE options are built specifically to exploit this confusion.\n\nGATE TRAP: Subject-verb agreement breaks down with collective nouns and phrases sitting between subject and verb. \"The list of items IS long\" is correct (the subject is \"list,\" singular, not \"items\"), and \"Each of the students HAS a book\" is correct (each is singular) — always identify the TRUE grammatical subject by mentally deleting the phrase in between, ignoring the noun closest to the verb.\n\nGATE TRAP: A percentage-point vs percent-change confusion appears even in verbal data-based reasoning passages (\"the pass rate rose from 40% to 50%\") — this is a rise of 10 percentage points but a 25% relative increase in the pass rate itself (10/40 × 100), and a critical-reasoning question can hinge on which of the two figures an option quotes.\n\nWORKED EXAMPLES\n\nExample 1 — Identifying a hidden assumption. Argument: \"The company should switch all its delivery trucks to electric vehicles because electric vehicles have lower fuel costs.\" Which assumption is necessary for this argument to hold?\n\n1. Slow route: read the argument and ask what would make the conclusion (switch to EVs) fail EVEN IF the premise (lower fuel costs) is true. If electric vehicles had a much higher upfront purchase price that outweighs the fuel savings, or could not travel the required delivery distance without recharging, the conclusion would collapse — so the argument silently assumes these problems do NOT exist, e.g., \"the total cost of ownership, not just fuel cost, favours electric vehicles\" or \"electric trucks can meet the delivery routes' range requirements.\"\n2. Fast route: apply the \"negate the assumption\" test directly to each answer option — if negating an option makes the argument's conclusion fall apart, that option IS a necessary assumption; if negating it leaves the conclusion untouched, it is not.\n3. Time saved: brainstorming assumptions freely from scratch can take a full minute per question; running the negation test on the 4-5 given options is a mechanical, fast check (15-20 seconds per option) that avoids missing the intended assumption.\n4. GATE TRAP check: an option like \"electric vehicles are better for the environment\" might be TRUE and even supportive in spirit, but it is not a NECESSARY assumption for this specific cost-based argument — the argument's conclusion does not depend on environmental benefit at all.\n\nExample 2 — Syllogism-style \"some\" trap embedded in critical reasoning. Statement: \"Some managers in the firm hold an MBA.\" Question: does this imply that some managers do not hold an MBA?\n\n1. Slow route: consider the most extreme case consistent with the statement — suppose ALL managers in the firm hold an MBA. Is \"some managers hold an MBA\" still a true statement in that extreme case? Yes, because \"some\" only requires at least one to be true, and having all of them true does not violate that. Since the extreme case (all managers hold an MBA) is consistent with the given statement, \"some managers do NOT hold an MBA\" cannot be forced to be true.\n2. Fast route: recall directly that \"Some A are B\" never implies \"Some A are not B\" as a fixed rule, and reject any option claiming otherwise without needing to construct the extreme-case scenario each time.\n3. Time saved: constructing the extreme-case scenario from scratch every time costs 20-30 seconds of careful reasoning; recalling the fixed rule rejects the trap option in under 5 seconds.\n4. GATE TRAP check: this exact wording (implying \"some...not\" from \"some\") is one of the most repeated traps across GATE years precisely because it feels intuitively true in everyday speech, where people usually say \"some\" to mean \"not all.\"\n\nExample 3 — Para-jumble using transition-word anchors. Arrange into a coherent paragraph: (P) This made the region highly attractive to traders. (Q) The ancient city was built at the junction of two major rivers. (R) However, frequent flooding eventually forced settlers to relocate. (S) As a result, it grew rapidly into a centre of commerce.\n\n1. Slow route: read all four sentences and try to construct meaning by guessing a plausible story order, checking each candidate order for sense: Q (city built at rivers) naturally sets the scene, so it should come first; P (\"This made the region attractive\") needs an antecedent for \"this,\" which is best supplied by Q's river junction, so P follows Q; S (\"As a result, it grew...\") follows naturally from P's \"attractive to traders\"; R (\"However, flooding...\") introduces a contrast to the growth described in S, so it comes last. Order: Q-P-S-R.\n2. Fast route: scan only for transition-anchor words first, before reading for meaning: \"This\" in P must refer to something concrete just stated, ruling P out as the opening sentence and pointing to Q (which gives the concrete fact \"junction of two rivers\") as its predecessor; \"As a result\" in S signals a direct consequence, pointing to P (the attractiveness) as its predecessor; \"However\" in R signals a contrast against the immediately preceding idea, which only makes sense after S's growth narrative. This chain of anchors alone reconstructs Q-P-S-R without needing to fully read for narrative sense.\n3. Time saved: reading for full narrative sense and testing multiple candidate orders can take 45-60 seconds; anchoring on \"This,\" \"As a result,\" and \"However\" directly chains the sentences in 15-20 seconds.\n4. GATE TRAP check: a student might place R second (right after Q) because flooding and rivers both relate to geography, but \"However\" demands a preceding positive/growth idea to contrast against, which only S provides — topical similarity is not the same as correct transition logic.\n\nExample 4 — Correlation vs causation in a data-based critical reasoning question. Passage: \"Cities that built more public libraries in the last decade also saw a rise in average literacy rates. Therefore, building public libraries increases literacy.\" Is this conclusion valid?\n\n1. Slow route: check whether any alternative explanation could produce the same observed pattern (libraries and literacy rising together) without libraries causing literacy. It is entirely possible that rising general prosperity in a city causes BOTH more library construction (cities spend more on public infrastructure when wealthier) AND more literacy (via better schools, more leisure reading time) — prosperity would be a hidden common cause, meaning the observed correlation does not prove the stated causal claim.\n2. Fast route: apply the fixed rule \"correlation never proves causation\" the instant a passage claims \"X and Y rose together, therefore X causes Y\" — immediately look among the answer options for one that proposes an alternative explanation (a third factor, or reversed causation: literate populations demanding more libraries) rather than trying to construct the alternative from scratch.\n3. Time saved: building a full alternative-explanation scenario unaided can take 30-40 seconds of creative thought; recognising the pattern instantly directs you straight to scanning options for \"third factor\" or \"reverse causation\" phrasing, cutting the time roughly in half.\n4. GATE TRAP check: an option stating \"this proves libraries cause literacy to rise\" restates the passage's own unproven leap and is a trap for anyone who reads too quickly and mistakes restatement for validation."; })();


(function(){ var t = window.GATE_DATA.questions['apti'].topics.find(function(t){return t.id==='apti-data-spatial';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n\nReading a chart without misreading axes. Every chart has an x-axis (usually the category or time, read left to right) and a y-axis (usually the measured value, read bottom to top) — before answering ANY question, read both axis labels and their units first, because a value plotted against \"sales in lakhs\" is not the same as \"sales in units,\" and a common trap is a y-axis that does NOT start at zero, which visually exaggerates small differences.\n\n1. Read the chart's title to know what is actually being measured.\n2. Read both axis labels and their units (percent, count, currency, thousands, lakhs).\n3. Check where the y-axis actually starts — if it does not start at 0, mentally correct for the visual exaggeration before comparing bar heights by eye.\n4. For a multi-series chart, match each colour or pattern in the legend to its series before reading any individual point.\n\nWhat \"percentage of total\" and \"ratio\" mean inside a data table. When a table gives values as percentages of some total, you cannot recover the actual numeric values unless the actual total (or one actual value) is also given somewhere — percentages and ratios are pure relationships, never absolute quantities, until anchored to a real number.\n\nactual value = (percentage / 100) × total\n\nWhat an average really is, and why it can mislead in charts. An average (mean) redistributes a total equally across all contributing items — Total ÷ Count — but a chart showing yearly averages can hide huge swings within a single year, and averaging several already-averaged figures together is valid only if each contributed with equal weight (equal group size or equal time period).\n\naverage = (sum of values) / (number of values)\n\nSpatial visualisation from zero — mirror images, water images, and folding. A mirror image flips a figure left-right (as if reflected in a vertical mirror placed to one side), while a water image flips it top-to-bottom (as if reflected in a horizontal surface below it, like a reflection in a pond) — these are NOT the same transformation, and confusing them is the most basic spatial-reasoning error. Folding questions (cube nets, paper folding with punched holes) test whether you can track a flat 2D layout accurately as it becomes a 3D shape, which is a skill built by physically tracing edges with a finger, not by \"just imagining it\" untrained.\n\nKEY: Mirror image reflects left-right (vertical mirror line); water image reflects top-to-bottom (horizontal mirror line) — for a clock face, a mirror image effectively shows the time as if reading it backwards through glass, while a water image shows it upside-down.\n\nKEY: In a pie chart, each slice's angle in degrees corresponds to its share of the whole: angle = (value / total) × 360. Convert an angle back to a percentage by dividing by 360 and multiplying by 100 — pie-chart questions frequently give the angle and expect you to find the percentage, or vice versa.\n\nKEY: In a line graph, the STEEPNESS of the segment between two points shows the rate of change, not the height of either point alone — a steep segment between two low points can represent a bigger jump than a flat segment between two high points.\n\nKEY: For \"approximate value\" data-interpretation questions, round the given numbers to convenient nearby values (e.g., 197 to 200, 48 to 50) before doing arithmetic — GATE options are usually spaced far enough apart that a sensibly rounded estimate lands unambiguously on the correct choice.\n\nKEY: When a table lists year-on-year percentage changes rather than actual values, you must chain them multiplicatively, not additively, to get overall change across several years — treat each year's change exactly like a successive-percentage-change problem.\n\nREMEMBER: Before comparing two bars, two pie slices, or two data points across different charts in the same question set, confirm they use the SAME unit and the SAME base (both in absolute numbers, or both as percentages of the same total) — comparing a percentage in one chart to an absolute count in another is meaningless.\n\nEVERY EDGE CASE\n\nGATE TRAP: A percentage-point change and a percent change are different numbers and data-interpretation questions are built specifically to test this. If market share rises from 20% to 30%, that is a rise of 10 percentage points, but a 50% relative increase in market share itself (10/20 × 100) — always check exactly which phrasing (\"percentage point\" vs \"percent\" or \"percentage increase\") the question uses before picking an option.\n\nGATE TRAP: Average of averages across unequal-sized groups (e.g., different years with different numbers of transactions, or different regions with different populations) is invalid unless you weight by the group size — a simple mean of several yearly average values silently assumes every year had the same number of underlying data points, which is rarely stated and rarely true.\n\nGATE TRAP: A ratio read off a chart (e.g., \"Product A sold twice as much as Product B\") tells you nothing about actual quantities unless anchored to one real figure elsewhere in the chart or table — do not assume round actual numbers just because the ratio looks clean.\n\nGATE TRAP: Truncated or broken y-axes (marked with a zig-zag break near the bottom) are used specifically to make small real differences look large — always check for a break symbol near the axis origin, and if present, treat visual bar-height comparisons with extra caution, relying on the actual labelled values instead.\n\nGATE TRAP: Dual-axis charts (two different y-axes, often one on the left in one unit and one on the right in a different unit) can make two completely unrelated-scale series look like they move together — always check which axis each series belongs to before claiming a trend or correlation between them, since matching visual slopes on different scales proves nothing about the real relationship between the series.\n\nGATE TRAP: A stacked bar chart's individual segment heights must be found by SUBTRACTING consecutive cumulative boundaries, not read directly off the y-axis — the top of the second segment gives the CUMULATIVE total of segments one and two, not the second segment's own value alone.\n\nGATE TRAP: Mirror-image and water-image questions on clocks and calendars require a specific arithmetic conversion, not just visual guessing. For a 12-hour clock, mirror time = 11 hours 60 minutes − actual time (equivalently, subtract the actual time from 11:60); water image similarly needs its own fixed relation, and applying the WRONG one of these two formulas is the single most common error in clock-reflection questions.\n\nmirror time = 11:60 - actual time\n\nGATE TRAP: When folding a cube net, faces that are ADJACENT in the flat net are almost never adjacent (touching) once folded, and faces separated by exactly one square in a straight strip of four are the ones that end up OPPOSITE each other — always identify the longest straight strip in the net first rather than trying to fold the entire net at once in your head.\n\nWORKED EXAMPLES\n\nExample 1 — Percentage-point vs percent-change in a data table. A company's defect rate fell from 8% to 5% of units produced. Find both the percentage-point drop and the percent decrease in the defect rate.\n\n1. Slow route: percentage-point drop = 8 − 5 = 3 percentage points, a simple subtraction of the two given percentages. Percent decrease (relative change) = (change / original) × 100 = (3/8) × 100 = 37.5%.\n2. Fast route: recognise instantly that \"percentage points\" always means simple subtraction of the two percentage figures (no need to divide by anything), while \"percent decrease/increase\" always means (difference / original value) × 100 — apply the correct formula the instant you see which phrase the question uses, without re-deriving which one applies.\n3. Time saved: without this fixed mapping, a student might compute (3/8)×100 for BOTH answers, wasting time re-checking which one the question wants, or worse, submit only one number when the question (or two separate options) needs the other — recognising the fixed vocabulary-to-formula mapping saves 20-30 seconds and prevents this exact mix-up.\n4. GATE TRAP check: an option offering \"3%\" as the percent decrease (confusing it with the percentage-point figure) is the standard trap answer choice in this exact question type.\n\nExample 2 — Weighted average across unequal groups in a data-interpretation set. A factory's average monthly output was 200 units for the first 8 months and 300 units for the remaining 4 months of the year. Find the average monthly output for the full year.\n\n1. Slow route: total output = (200 × 8) + (300 × 4) = 1600 + 1200 = 2800 units over 12 months. Average = 2800/12 = 233.33 units per month.\n2. Fast route: apply the weighted average formula directly with the month-counts as weights: (200×8 + 300×4)/(8+4) = 2800/12 = 233.33, skipping the intermediate \"total output\" sentence and going straight from the weighted-sum template to the number.\n3. Time saved: both routes use the same arithmetic, but naming it as a weighted-average template immediately (rather than re-deriving \"total ÷ months\" from scratch) saves the 10-15 seconds spent second-guessing which total and which count to divide by, especially useful when a data-interpretation set has multiple similar sub-questions to solve quickly.\n4. GATE TRAP check: the plain average of the two monthly figures, (200+300)/2 = 250, is WRONG here because it silently treats the 8-month period and the 4-month period as equal in weight — a classic average-of-averages trap embedded directly in a data-interpretation question.\n\nExample 3 — Clock mirror-image conversion. A clock's reflection in a mirror shows the time as 4:20. Find the actual time.\n\n1. Slow route: actual time = 11 hours 60 minutes − mirror time. Subtract minutes first: 60 − 20 = 40 minutes. Subtract hours: 11 − 4 = 7 hours. So the actual time is 7:40.\n2. Fast route: apply the fixed formula \"actual = 11:60 − mirror\" as a single mechanical subtraction (minutes column first, then hours column, exactly like subtracting two clock-times), without re-deriving why 11:60 is the correct constant to subtract from each time.\n3. Time saved: re-deriving the mirror-time relationship geometrically (imagining the clock face and where each hand would appear reflected) can take 40-60 seconds under exam pressure; applying the memorised 11:60-minus formula directly gives the answer in 10-15 seconds.\n4. GATE TRAP check: a student who instead computes 12:00 − 4:20 = 7:40 by accident gets the same answer here only by coincidence of these particular numbers — the correct constant is 11 hours 60 minutes (not a plain 12:00) because subtracting minutes and hours as separate columns from a literal \"12:00\" requires an awkward borrow; always use the 11:60 form to keep the subtraction clean in both columns.\n\nExample 4 — Cube net folding, opposite faces from a straight strip. A cube's net has four squares in a straight horizontal strip labelled, from left to right, P, Q, R, S, with a fifth square T attached above Q and a sixth square U attached below R. Which face is opposite T?\n\n1. Slow route: mentally fold the net into a cube — the strip of four squares P, Q, R, S wraps exactly halfway around the cube when folded, which means P (1st) and R (3rd) land on opposite faces, and Q (2nd) and S (4th) land on opposite faces. That accounts for P-R and Q-S as two opposite pairs, leaving T and U as the third pair, so T is opposite U.\n2. Fast route: recall the fixed rule directly — for any straight strip of four consecutive squares in a net, the 1st and 3rd squares become opposite, and the 2nd and 4th become opposite; any squares left over after applying this rule to the longest strip must be opposite each other by elimination (since a cube has exactly 3 opposite pairs).\n3. Time saved: physically visualising a paper net folding step by step in your head can take 45-90 seconds and is highly error-prone without practice; applying the \"1st-3rd, 2nd-4th\" rule to the longest strip and using elimination for the rest gives the answer in 15-20 seconds with far less error risk.\n4. GATE TRAP check: T is attached directly above Q in the flat net, which might tempt you into guessing \"T is opposite Q\" by proximity — but Q's true opposite is S (from the 2nd-4th rule on the main strip), and T's opposite is determined only by elimination once P-R and Q-S are fixed, since being adjacent in the flat net says nothing about being opposite after folding."; })();
