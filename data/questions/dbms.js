...
(function(){ var t = window.GATE_DATA.questions['dbms'].topics.find(function(t){return t.id==='dbms-er';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n• Relation. A **relation** is just a table: a set of rows, each row describing one real-world thing. In formal terms, a relation is a set of tuples over a fixed set of attributes.\n• Tuple. A **tuple** is one row of a relation -- one specific record, e.g. (101, 'Asha', 'CSE').\n• Attribute. An **attribute** is one column of a relation -- a named property every tuple has a value for, e.g. RollNo, Name, Dept. Each attribute has a **domain**, the set of legal values it may take (e.g. Age's domain might be integers 0-120).\n• Entity, entity set, relationship. In ER modelling (a design tool used BEFORE tables exist), an **entity** is a real-world object (a specific student), an **entity set** is a collection of similar entities (all students), and a **relationship** connects entities from one or more entity sets (a Student ENROLLS_IN a Course).\n\nEVERY EDGE CASE\n• Total vs partial participation. **Total participation** means EVERY entity in that entity set MUST take part in the relationship (drawn as a double line in ER diagrams) -- e.g. every Employee must WORK_IN some Department. **Partial participation** means some entities may not participate at all (single line) -- e.g. not every Employee MANAGES a project.\nGATE TRAP: participation constraints (total/partial) are about WHETHER an entity must appear at all in the relationship, and are completely independent from cardinality ratio (how MANY it can appear with) -- a relationship can be 1:1 with partial participation on both sides, or many:many with total participation on one side. Do not conflate the two.\n• Cardinality ratios: 1:1, 1:N, N:1, M:N. These describe how many entities on ONE side can be linked to how many on the OTHER side. In a 1:N relationship, when converting to tables, the foreign key goes on the 'N' (many) side's table, referencing the '1' side's primary key. In an M:N relationship, NEITHER side's table can hold the foreign key alone -- you must create a SEPARATE junction/bridge table holding the primary keys of both sides (plus any relationship attributes).\nGATE TRAP: trying to fold an M:N relationship's foreign key into one of the two original entity tables (instead of creating a new table) always causes either data loss or unwanted repetition -- this is the most common ER-to-relational mapping mistake tested.\n• Weak entity sets. A **weak entity** has no primary key of its own sufficient to uniquely identify it; it depends on a related **strong (identifying) entity** and borrows that entity's key, combined with its own **partial key** (also called discriminator), to form a full key. E.g. a Dependent (spouse/child) of an Employee is identified only in combination with the Employee's key (EmpID + DependentName). Weak entities always have TOTAL participation in their identifying relationship (a dependent cannot exist without its owning employee).\nGATE TRAP: the partial key alone (e.g. DependentName) is NOT unique across the whole weak entity set -- two different employees could each have a dependent named 'Ravi'. Only (EmpID, DependentName) together is a valid key for the weak entity's table.\n• Multivalued and derived attributes. A **multivalued attribute** (e.g. a Person's PhoneNumbers, shown with a double oval) can take more than one value per entity, and when mapped to relations it MUST become its own separate table (with a foreign key back to the owning entity) -- it cannot stay as one column. A **derived attribute** (e.g. Age, computable from DateOfBirth) is NOT stored physically -- it is computed on demand and normally does not even get a column in the mapped table.\n• Aggregation. **Aggregation** treats an entire relationship (with its participating entities) as a higher-level abstract entity so that it, in turn, can participate in another relationship. This is needed when a relationship itself needs to relate to a third entity set, and drawing a relationship directly between another relationship and an entity is not allowed in ER notation without aggregation.\n• Specialization / generalization and constraints. **Specialization** splits a higher-level entity set into lower-level subtypes based on distinguishing features (e.g. Employee specializes into Manager, Engineer). Two independent constraints apply: **disjoint vs overlapping** (can one entity belong to more than one subtype at once?) and **total vs partial** (must every higher-level entity belong to SOME subtype?). A common source of confusion is conflating these two orthogonal constraints -- disjoint/overlapping is about subtype MEMBERSHIP OVERLAP; total/partial is about whether membership is MANDATORY.\nGATE TRAP: 'disjoint, total' means every entity is in EXACTLY one subtype; 'overlapping, partial' means an entity MAY be in zero, one, or several subtypes at once -- exam questions often give a scenario description and ask you to name the correct pair of constraints, so read for both signals independently.\n\nWORKED EXAMPLE -- full cardinality + participation to relational schema\nConsider entity sets STUDENT (key: RollNo) and COURSE (key: CourseCode), related by ENROLLS (attributes: Grade), where the cardinality is M:N (a student can enroll in many courses, a course can have many students), and STUDENT has total participation in ENROLLS (every student must enroll in at least one course) while COURSE has partial participation (a newly added course may have zero students so far).\n1. step: Because the relationship is M:N, create a separate table ENROLLS(RollNo, CourseCode, Grade), with RollNo referencing STUDENT and CourseCode referencing COURSE as foreign keys.\n2. step: The primary key of the new ENROLLS table is the composite (RollNo, CourseCode) -- this pair together uniquely identifies one enrollment record, but neither column alone does.\n3. step: Total participation of STUDENT is a business-rule / application-level constraint (every student row must have at least one matching ENROLLS row) -- standard relational schemas cannot directly enforce 'at least one' with a simple foreign key, so this is usually noted as an application constraint or enforced via a trigger, NOT something visible purely from the table structure.\n4. step: STUDENT and COURSE remain separate tables: STUDENT(RollNo, Name, ...) and COURSE(CourseCode, Title, ...) -- unaffected by the M:N relationship except for being referenced by ENROLLS.\nKEY: M:N relationship => always a 3rd bridge table; 1:N relationship => foreign key placed in the 'many' side's table; 1:1 relationship => foreign key can go on EITHER side (commonly placed on the side with total participation, to avoid NULLs).\n\nWORKED EXAMPLE -- weak entity mapping\nEntity set LOAN_PAYMENT is a weak entity, dependent on the strong entity set LOAN (key: LoanNumber) via identifying relationship PAYMENT_OF, with LOAN_PAYMENT's own partial key PaymentNumber (which restarts from 1 for every different loan) and attribute PaymentDate.\n1. step: The identifying relationship PAYMENT_OF has total participation from LOAN_PAYMENT (every payment record must belong to some loan) -- this is a rule for ALL weak entities.\n2. step: Map LOAN_PAYMENT to a table: LOAN_PAYMENT(LoanNumber, PaymentNumber, PaymentDate), where LoanNumber is a foreign key to LOAN.\n3. step: The primary key of this table is the composite (LoanNumber, PaymentNumber) -- PaymentNumber ALONE is not unique because it restarts at 1 for every loan, so a payment #1 exists for many different loans simultaneously; only the pair is globally unique.\nGATE TRAP: a very common ER wrong-answer choice is to claim the weak entity's partial key (PaymentNumber alone) can serve as its primary key -- always check whether the partial key repeats across different owning strong entities; if it can, the owning entity's key MUST be included.\n\nWORKED EXAMPLE -- specialization constraint identification\nAn ACCOUNT entity set specializes into SAVINGS_ACCOUNT and CURRENT_ACCOUNT. The bank's rule: every account must be exactly one of the two types, never both, and never neither.\n1. step: 'Never both' identifies the **disjoint** constraint (subtypes do not overlap).\n2. step: 'Never neither' (every account must fall into some subtype) identifies the **total** constraint.\n3. step: Combined ER notation: disjoint, total specialization, often drawn with a circled 'd' and a double line from ACCOUNT down to the specialization circle.\nKEY: four possible combinations exist -- (disjoint, total), (disjoint, partial), (overlapping, total), (overlapping, partial) -- a scenario description always signals BOTH independently, so extract them as two separate yes/no answers rather than guessing one combined label.\n";
})();

(function(){ var t = window.GATE_DATA.questions['dbms'].topics.find(function(t){return t.id==='dbms-ra-sql';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n• Relational algebra. **Relational algebra** is a procedural query language: a set of operators (select, project, join, union, etc.) that take one or more relations as input and produce a new relation as output -- SQL queries are, under the hood, translated into relational algebra expressions for execution.\n• Select (sigma) vs Project (pi). **Select**, written sigma_condition(R), picks ROWS of R that satisfy a condition (like SQL's WHERE) -- it never changes the columns. **Project**, written pi_attributes(R), picks COLUMNS of R (like choosing which fields to keep) -- and because the result is a set, project automatically removes duplicate rows that result from dropping columns.\n• NULL. **NULL** means 'unknown' or 'not applicable' -- it is NOT the number 0, NOT an empty string, and NOT a special value you can compare with '='. SQL uses three-valued logic: any comparison involving NULL (NULL = 5, NULL > 3, etc.) evaluates to UNKNOWN, not TRUE or FALSE.\n• Set vs multiset (bag). A pure **set** never contains duplicate elements. SQL tables are technically **multisets (bags)** -- by default, duplicate rows ARE allowed and ARE kept, unless you explicitly say DISTINCT.\n\nEVERY EDGE CASE\n• Three-valued logic and WHERE. A WHERE clause keeps only rows where the condition evaluates to TRUE -- rows evaluating to FALSE or UNKNOWN are both excluded. So `WHERE salary > 5000` silently drops any row where salary IS NULL, even though 'salary > 5000' being unknown is not the same as it being false.\nGATE TRAP: to explicitly find rows with unknown/missing values you must write `WHERE salary IS NULL` -- writing `WHERE salary = NULL` ALWAYS evaluates to UNKNOWN (never TRUE) and returns zero rows, no matter what data exists, because '=' comparison with NULL never succeeds.\n• NULL and aggregate functions. `COUNT(*)` counts ALL rows including those with NULLs in any column. `COUNT(column_name)` counts only rows where that SPECIFIC column is NOT NULL. `SUM`, `AVG`, `MAX`, `MIN` on a column all IGNORE NULL values entirely when computing the result (they do not treat NULL as 0).\nGATE TRAP: `AVG(salary)` when 2 out of 5 rows have NULL salary computes the average over only the 3 non-null rows, NOT sum/5 -- a very common numeric trap. Also, if EVERY value in a column is NULL, `SUM`/`AVG`/`MAX`/`MIN` return NULL (not 0), while `COUNT(column)` correctly returns 0.\n• NULL in GROUP BY. SQL treats all NULL values in a grouping column as belonging to ONE single group together (even though NULL = NULL is UNKNOWN, not TRUE, GROUP BY uses a special 'is not distinct from' style comparison for grouping purposes) -- this is an exception to normal 3-valued-logic comparison rules.\n• Set operators (UNION, INTERSECT, EXCEPT/MINUS) vs bag semantics. Plain `UNION`, `INTERSECT`, `EXCEPT` in SQL automatically remove duplicates (true set semantics) -- but `UNION ALL`, `INTERSECT ALL`, `EXCEPT ALL` preserve duplicates (bag/multiset semantics), keeping the exact multiplicity rules (e.g. `INTERSECT ALL` keeps min(count in R, count in S) copies of each tuple).\nGATE TRAP: relational algebra's set operators (union, intersection, set-difference) are defined on pure SETS and REQUIRE the two relations to be UNION-COMPATIBLE (same number of attributes, corresponding domains) -- applying union to relations with a different number of columns, or where corresponding columns have incompatible types, is undefined/invalid, a frequent 'spot the invalid RA expression' trap.\n• Division operator. Relational algebra's DIVISION, R / S, returns tuples of R (restricted to attributes not in S) that are associated with EVERY tuple in S -- the classic 'find X that relates to ALL Y' pattern (e.g. find students who have taken ALL courses that a given course-list contains). This is one of GATE's favourite RA operators precisely because it has no single, simple SQL keyword equivalent -- it must be simulated using double NOT EXISTS.\nFORMULA: R / S = pi_(R-S)(R) - pi_(R-S)( (pi_(R-S)(R) x S) - R )\n• Outer joins and NULL padding. A **natural join** (or inner join) keeps only rows that match on both sides, dropping unmatched rows entirely. A **left outer join** keeps every row of the LEFT relation, padding with NULLs for right-side columns when there is no match; a **right outer join** does the mirror; a **full outer join** keeps unmatched rows from BOTH sides, each padded with NULLs on the side that didn't match.\nGATE TRAP: after an outer join, any subsequent WHERE condition that touches the padded (potentially NULL) columns can silently discard the very outer-joined rows you wanted to keep, because comparisons with NULL are UNKNOWN -- this is why outer-join filtering conditions are usually written in the JOIN's ON clause, not a later WHERE.\n• Duplicate elimination cost trade-off. Because RA's project (pi) is technically pure-set based (removes duplicates) but real engines execute over bags for speed, GATE sometimes tests 'does this relational algebra expression, translated literally to SQL, produce the SAME result as SQL's default (bag) behavior' -- the answer is often NO exactly because of this duplicate-removal mismatch, unless DISTINCT is added.\n\nWORKED EXAMPLE -- NULL effect on COUNT and AVG\nTable MARKS(RollNo, Score) has 5 rows: (1,80), (2,NULL), (3,60), (4,NULL), (5,90).\n1. step: `SELECT COUNT(*) FROM MARKS` counts all rows regardless of NULLs => 5.\n2. step: `SELECT COUNT(Score) FROM MARKS` counts only non-NULL Score values => 3 (rows 1,3,5).\n3. step: `SELECT AVG(Score) FROM MARKS` sums only non-NULL scores (80+60+90=230) and divides by the COUNT of non-NULL scores (3), giving 230/3 = 76.67 -- NOT 230/5=46.\nGATE TRAP: a wrong-answer choice computing AVG as sum/(total row count including NULLs) is exactly the trap this question tests -- always divide by the count of NON-NULL values for AVG.\n\nWORKED EXAMPLE -- set vs bag UNION\nRelation R = {(1,'a'), (2,'b'), (2,'b')} (a bag with a duplicate) and S = {(2,'b'), (3,'c')}.\n1. step: `R UNION S` (set semantics) first conceptually treats R as the set {(1,'a'),(2,'b')} (removing R's internal duplicate) and S as {(2,'b'),(3,'c')}, then unions and removes duplicates across both: result = {(1,'a'), (2,'b'), (3,'c')} -- 3 rows, (2,'b') appears once even though it existed in both operands.\n2. step: `R UNION ALL S` (bag semantics) keeps everything: (1,'a'), (2,'b'), (2,'b') [from R, both copies], (2,'b') [from S], (3,'c') -- 5 rows total, with (2,'b') appearing 3 times.\nKEY: plain UNION/INTERSECT/EXCEPT = set semantics (dedupe); UNION ALL/INTERSECT ALL/EXCEPT ALL = bag semantics (preserve multiplicities) -- INTERSECT ALL keeps min(count_R, count_S) copies, EXCEPT ALL keeps max(0, count_R - count_S) copies of each distinct tuple.\n\nWORKED EXAMPLE -- relational algebra DIVISION\nEnrolled(RollNo, CourseCode) records which students took which courses. Required = {C1, C2} (a small relation listing exactly two course codes). We want every student who has taken BOTH C1 and C2 (i.e. all courses in Required).\n1. step: The RA expression is Enrolled / Required, dividing on the shared attribute CourseCode.\n2. step: Conceptually, for each candidate RollNo, check whether the set of CourseCodes that RollNo is enrolled in is a SUPERSET of {C1, C2}; if RollNo took C1, C2, and also C3, it still qualifies (division needs 'at least all of', not 'exactly').\n3. step: Suppose Enrolled = {(101,C1),(101,C2),(101,C3),(102,C1),(103,C1),(103,C2)}. Student 101 has both C1 and C2 (plus extra C3) -> qualifies. Student 102 has only C1 -> does NOT qualify (missing C2). Student 103 has exactly C1 and C2 -> qualifies. Result of Enrolled/Required = {101, 103}.\nGATE TRAP: division answers 'who relates to ALL of S', not 'who relates to ONLY S' -- a student enrolled in extra courses beyond the required set is still included; do not exclude row 101 just because it has additional CourseCode C3.\n";
})();

(function(){ var t = window.GATE_DATA.questions['dbms'].topics.find(function(t){return t.id==='dbms-normalization';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n• Functional dependency (FD). A **functional dependency** X -> Y means: if two tuples agree on all attributes in X, they must also agree on all attributes in Y -- X 'determines' Y. E.g. RollNo -> Name means no two students with the same RollNo can have different Names.\n• Candidate key. A **candidate key** is a MINIMAL set of attributes whose closure (everything it functionally determines) includes ALL attributes of the relation -- minimal means removing even one attribute from the set breaks that property. A relation can have several candidate keys; one is chosen as the **primary key**.\n• Prime vs non-prime attribute. A **prime attribute** is any attribute that is part of AT LEAST ONE candidate key. A **non-prime attribute** is part of NO candidate key.\n• Normalization. **Normalization** is the process of decomposing a relation into smaller relations to remove redundancy and the update/insert/delete anomalies that redundancy causes, guided by a series of increasingly strict rules called normal forms (1NF, 2NF, 3NF, BCNF, ...).\n\nEVERY EDGE CASE\n• Attribute closure algorithm (X+). To compute X+ (the closure of attribute set X under a set of FDs): start with result = X; repeatedly scan every FD (lhs -> rhs); if lhs is a subset of the current result, add all of rhs to result; repeat until no FD adds anything new.\n1. step: result = X.\n2. step: for each FD (lhs -> rhs) in F: if lhs subset-of result, then result = result union rhs.\n3. step: repeat step 2 until result stops changing (a full pass adds nothing new).\n4. step: the final result is X+.\nGATE TRAP: you must keep looping over ALL the FDs repeatedly (not just once) because adding new attributes can suddenly make the LHS of a previously-unusable FD satisfied -- stopping after one pass is the most common closure-computation mistake.\n• Partial dependency (violates 2NF). A **partial dependency** exists when a NON-PRIME attribute depends on only PART of a composite candidate key, not the whole key. E.g. if (StudentID, CourseID) is the key and StudentName depends on StudentID alone, that's a partial dependency -- 2NF requires every non-prime attribute to depend on the WHOLE key.\nGATE TRAP: 2NF violations can ONLY happen when the candidate key is COMPOSITE (more than one attribute) -- a relation with an entirely single-attribute candidate key is automatically already in 2NF (there is no 'partial' key to depend on).\n• Transitive dependency (violates 3NF). A **transitive dependency** exists when a non-prime attribute depends on another non-prime attribute, which in turn depends on the key -- i.e. Key -> A -> B, where A and B are both non-prime and A does not itself contain a candidate key. E.g. StudentID -> DeptID -> DeptName: DeptName depends transitively on StudentID through DeptID.\n• The exact 3NF definition (and its prime-attribute escape clause). A relation is in 3NF if for every non-trivial FD X -> A, EITHER (a) X is a superkey, OR (b) A is a PRIME attribute. Clause (b) is a subtle exception: it means a transitive-looking dependency is still ALLOWED in 3NF if the dependent attribute A happens to be part of some candidate key.\nGATE TRAP: this prime-attribute escape clause is exactly what distinguishes 3NF from BCNF -- BCNF drops clause (b) entirely and only allows condition (a). Many GATE questions construct a relation that is in 3NF but NOT in BCNF specifically by exploiting an FD where A is prime; missing this exception causes wrong 3NF/BCNF classification.\n• BCNF definition and dependency-preservation trade-off. A relation is in **BCNF** if for every non-trivial FD X -> A, X MUST be a superkey (no exception for prime attributes). BCNF is strictly stronger (or equal) than 3NF and removes ALL redundancy from functional dependencies, but decomposing into BCNF is NOT always possible while also preserving every original functional dependency -- sometimes you must accept losing dependency preservation to achieve BCNF.\nGATE TRAP: 3NF ALWAYS guarantees both lossless-join decomposition AND dependency preservation simultaneously; BCNF ALWAYS guarantees lossless-join, but CANNOT always guarantee dependency preservation. If a question asks 'which normal form guarantees both lossless join and dependency preservation together, always', the safe universal answer is 3NF, not BCNF.\n• Lossless-join decomposition test. Decomposing R into R1 and R2 is **lossless** if and only if the common attributes (R1 intersect R2) form a superkey of AT LEAST ONE of R1 or R2 -- i.e. (R1 ∩ R2) -> R1 or (R1 ∩ R2) -> R2 holds.\nGATE TRAP: forgetting to check both directions (only checking whether it determines R1, and not also checking R2) can wrongly mark a lossy decomposition as lossless, or vice versa.\n• Multivalued dependency and 4NF. A **multivalued dependency** X ->> Y (independent of any functional dependency) exists when, for a fixed value of X, the set of values of Y is independent of the values of every other attribute Z. This causes redundancy even in a BCNF relation if not addressed -- **4NF** requires that every non-trivial multivalued dependency X ->> Y also has X as a superkey.\n\nWORKED EXAMPLE -- full attribute closure and candidate-key derivation\nRelation R(A,B,C,D,E) with FDs: F = { A -> BC, CD -> E, B -> D, E -> A }.\n1. step: Compute A+. Start {A}. A->BC applies (A subset of {A}): add B,C => {A,B,C}. B->D applies: add D => {A,B,C,D}. CD->E applies (C,D both present): add E => {A,B,C,D,E}. No more FDs add anything. A+ = {A,B,C,D,E} = all attributes, so {A} is a SUPERKEY.\n2. step: Check minimality of {A}: since it's a single attribute, it's automatically minimal (no smaller non-empty subset exists) -- so {A} is a CANDIDATE KEY.\n3. step: Compute E+. Start {E}. E->A applies: add A => {E,A}. A->BC applies: add B,C => {E,A,B,C}. B->D applies: add D => {E,A,B,C,D}. E+ = all attributes, so {E} is also a candidate key (single attribute, automatically minimal).\n4. step: Check remaining single attributes: B+ = {B,D} (only B->D applies, D+ nor C+ contribute more) -- not a superkey. C+ = {C} alone -- not a superkey. D+ = {D} alone -- not a superkey. So no other single-attribute candidate key exists, and since A and E already independently form keys, no LARGER combination needs checking for minimality purposes (any set containing A or E is a superkey but not minimal).\n5. step: Final candidate keys = {A} and {E}. Prime attributes = {A, E}. Non-prime attributes = {B, C, D}.\nKEY: an attribute that never appears on the right-hand side of ANY given FD can never be derived from anything else, so it MUST belong to every candidate key (a quick shortcut check before running full closures).\n\nWORKED EXAMPLE -- classifying partial vs transitive dependency and normal form\nUsing the R(A,B,C,D,E) result above (candidate keys {A} and {E}, prime = {A,E}, non-prime = {B,C,D}): consider the FD B -> D.\n1. step: Is this a 2NF violation (partial dependency)? Partial dependency requires a COMPOSITE candidate key with a non-prime attribute depending on part of it. Since the candidate keys here are single attributes ({A} and {E}), there is NO composite key, so 2NF violations of this type cannot occur here at all -- B->D is not 'partial' by definition.\n2. step: Is B->D a transitive dependency issue for 3NF? Check the 3NF condition for B->D: is B a superkey? B+ = {B,D}, not all attributes, so B is NOT a superkey. Is D (the RHS) a prime attribute? D is non-prime (not in {A,E}). Since neither condition holds, B->D VIOLATES 3NF.\n3. step: Because B->D violates 3NF (B is not a superkey, D is not prime), R is NOT in 3NF, and therefore also not in BCNF (BCNF is strictly stronger).\nGATE TRAP: even without a composite key, a relation can still violate 3NF/BCNF purely through a non-superkey determining a non-prime attribute -- 'no composite key exists' does NOT automatically mean the relation is well-normalized; always separately check every single FD against the superkey-or-prime rule.\n\nWORKED EXAMPLE -- BCNF decomposition that loses dependency preservation\nRelation R(A,B,C) with FDs: AB -> C and C -> B. Candidate keys: AB+ = {A,B,C} (superkey, and minimal since neither A+ nor B+ alone reaches C) so AB is a candidate key; also check AC+ = {A,C,B} (via C->B) = all attributes, and it is minimal (A+ alone and C+ alone don't cover everything), so AC is also a candidate key. Prime attributes = {A,B,C} (every attribute is prime here).\n1. step: Check BCNF for C -> B: is C a superkey? C+ = {C,B} only (C->B applies, nothing else), not all of R, so C is NOT a superkey -- this FD VIOLATES BCNF (note: it does NOT violate 3NF, since B is prime here -- 3NF's escape clause saves it).\n2. step: To achieve BCNF, decompose R on the violating FD C->B: create R1(C,B) [the FD's own attributes] and R2(A,C) [remaining attributes plus the determinant C].\n3. step: Check lossless join: R1 ∩ R2 = {C}; does C -> R1 or C -> R2? C+ = {C,B} ⊇ R1={B,C}, yes -- so the decomposition is lossless.\n4. step: Check dependency preservation: the original FD AB->C -- can it be verified by checking FDs within R1 and R2 alone (without recombining via a join)? R2(A,C) contains A and C but AB->C needs B, which is not present in R2; R1(C,B) doesn't have A either. AB->C cannot be enforced by checking either fragment in isolation -- it is LOST.\nGATE TRAP: this is the textbook demonstration that BCNF decomposition guarantees lossless-join but can sacrifice dependency preservation -- if a question asks you to normalize R(A,B,C) with these exact FDs to BCNF, the 'correct' answer must acknowledge that AB->C becomes an inter-relation constraint that the DBMS can no longer check with simple per-table FDs alone.\n";
})();

(function(){ var t = window.GATE_DATA.questions['dbms'].topics.find(function(t){return t.id==='dbms-indexing';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n• Block (page). A **block** (or page) is the fixed-size unit of data a database reads/writes from disk in one I/O operation, typically a few KB. Disk I/O is the slow part of database operations, so performance is usually measured in NUMBER OF BLOCK ACCESSES, not raw time.\n• Index. An **index** is an auxiliary data structure built on one or more columns of a table that lets the database find matching rows without scanning every row -- like a book's index letting you jump to a page instead of reading cover to cover.\n• Search key. The **search key** of an index is the column (or columns) the index is built on -- NOT necessarily the same as the table's primary key (you can index any column you frequently search on).\n• Pointer. In this context, a **pointer** is a block address (or a record address) stored inside an index entry, telling the database where the actual data physically lives.\n\nEVERY EDGE CASE\n• Primary vs secondary index (based on file ordering, not on 'primary key'). A **primary index** is built on the field the DATA FILE is physically SORTED by (also called a clustering index in some texts) -- there is at most one primary index per file. A **secondary index** is built on a field the file is NOT sorted by -- you can have many secondary indexes on the same file.\nGATE TRAP: 'primary index' here refers to sort order, not to the table's declared PRIMARY KEY constraint -- a table's primary key column, if the file isn't physically sorted on it, would need a SECONDARY index, not a primary one. Don't conflate the SQL concept of PRIMARY KEY with the file-organization concept of PRIMARY INDEX.\n• Dense vs sparse index. A **dense index** has ONE index entry for EVERY search-key value in the data file (every record gets an entry) -- works for both sorted and unsorted data files, and works for any index (primary or secondary). A **sparse index** has an index entry for only SOME records (typically one per BLOCK, pointing to the first record of that block) -- this ONLY works when the data file is sorted on the index's search key, because it relies on scanning forward from the pointed block to find the exact record.\nGATE TRAP: sparse indexes CANNOT be built as secondary indexes on an unsorted file -- since the underlying file isn't sorted by that key, one entry per block gives no way to know which nearby records might match. Dense indexes have no such restriction but need more storage.\n• Multilevel index. When even the (dense or sparse) index itself becomes too large to fit in memory in one piece, you build a SECOND-level sparse index over the FIRST-level index's blocks, and so on -- reducing the number of block accesses needed to reach a matching record, at the cost of extra levels to traverse (each level adds exactly 1 more block access for a lookup, but the total blocks scanned shrinks logarithmically for the top levels).\n• B+-tree order can differ between internal and leaf nodes. A B+-tree's **order p** is often defined as: an internal node holds up to p pointers and (p-1) keys, sized to fit one block. A LEAF node, however, additionally needs a POINTER TO THE NEXT LEAF (for fast sequential/range scans) and often stores actual RECORD pointers (which can be a different size than internal block pointers) -- so the maximum number of (key, pointer) entries that fit in a leaf block is generally a DIFFERENT number from the internal node's order, unless the problem states all pointers are the same size.\nGATE TRAP: assuming internal-node order and leaf-node order are automatically equal is a frequent mistake -- always redo the block-size arithmetic separately for leaf nodes (accounting for the extra next-leaf pointer and possibly different record-pointer size) rather than reusing the internal node's order number.\nFORMULA: Internal node order n satisfies n x Pblock + (n-1) x Vkey <= BlockSize. Leaf node order m satisfies m x Precord + (m-1) x Vkey + Pblock <= BlockSize (the extra +Pblock term is the next-leaf sibling pointer).\n• B+-tree height and search cost. For a B+-tree of order p (max p pointers per internal node) holding N leaf-level key entries, the number of levels (and therefore block accesses for an exact-match search) is roughly ceil(log_ceil(p/2)(N)) using the MINIMUM fill factor (each non-root node must be at least half full) -- GATE questions often ask for BEST CASE (using maximum fanout p) vs WORST CASE (using minimum fanout ceil(p/2)) height, and these give DIFFERENT numeric answers for the same tree.\nGATE TRAP: 'minimum number of levels' uses the MAXIMUM branching factor (p), while 'maximum number of levels' uses the MINIMUM branching factor (ceil(p/2), the half-full guarantee) -- swapping which bound goes with which extreme is a very common error.\n• B+-tree vs B-tree. A plain **B-tree** stores actual data/record pointers in INTERNAL nodes too (not just leaves), so a search can terminate early at a non-leaf node -- but this makes sequential range scans harder (no linked leaf chain) and wastes internal-node space that could otherwise hold more keys for branching. A **B+-tree** keeps ALL data pointers only in leaves (internal nodes hold only keys for navigation) and links leaves together, making range queries fast via one sequential scan of the leaf chain -- this is why almost all real database indexes use B+-trees, not plain B-trees.\n• Hashing edge cases: overflow and skew. In (static) hashing, if the hash function maps too many records to the same bucket beyond its capacity, an OVERFLOW CHAIN is needed, degrading search performance for that bucket toward linear scan. **Extendable (dynamic) hashing** solves the problem of a FIXED number of buckets becoming too small as data grows, by doubling the directory (adding one more bit of the hash value examined) only for the parts of the hash space that actually need it -- avoiding a full, expensive rehash of every record at once, unlike simple/static hashing which requires a full rehash when resized.\n\nWORKED EXAMPLE -- B+-tree order differing between internal and leaf nodes\nBlock size B = 1024 bytes. Key size Vkey = 14 bytes. Block pointer size Pblock = 8 bytes. Record pointer size Precord = 7 bytes (data record pointers are smaller than block pointers here, a scenario GATE sometimes sets up explicitly).\n1. step: Internal node order n: n x 8 + (n-1) x 14 <= 1024 => n x (8+14) <= 1024+14 => n <= 1038/22 = 47.18. Taking the floor, n = 47 -- so an internal node holds up to 47 pointers and 46 keys.\n2. step: Leaf node order m: m x 7 + (m-1) x 14 + 8 <= 1024 (the +8 is the next-leaf sibling pointer) => m x (7+14) <= 1024 - 8 + 14 => m <= 1030/21 = 49.05. Taking the floor, m = 49 -- so a leaf node holds up to 49 (key, record-pointer) pairs plus the one next-leaf pointer.\n3. step: Compare -- internal order (47) is DIFFERENT from leaf order (49) purely because record pointers (7 bytes) are smaller than block pointers (8 bytes) and the extra next-leaf-pointer term shifts the arithmetic differently. A question assuming both must be equal would get either wrong.\nKEY: always redo the inequality separately for internal nodes (n x Pblock + (n-1) x Vkey <= B) and leaf nodes (m x Precord + (m-1) x Vkey + Pblock <= B) -- never assume they match unless the problem explicitly states all pointer types are the same size.\n\nWORKED EXAMPLE -- best-case vs worst-case B+-tree height\nA B+-tree has order (max fanout) p = 50 and must index N = 1,000,000 leaf-level records.\n1. step: BEST CASE height (maximum branching, every node completely full at p=50 pointers): number of levels above the leaf level h satisfies p^h >= N/leafCapacity roughly; approximating leaf capacity ~ p as well for a quick estimate, height ~ log_50(1,000,000) ~ 3.5, so about 4 levels total including leaves in the best case.\n2. step: WORST CASE height (minimum branching, every non-root node only half full, i.e. ceil(50/2)=25 pointers): height instead uses log_25(1,000,000) ~ 4.3, so about 5 levels -- MORE levels than the best case for the SAME data, because each half-full node covers fewer records per level.\nGATE TRAP: when a question asks for 'the minimum possible height', it wants the scenario that lets the tree be AS SHALLOW as possible, which happens when nodes are packed as full as possible (using p, the maximum fanout) -- not the half-full minimum-fill number; conversely 'maximum possible height' uses the half-full (ceil(p/2)) minimum fanout, since less-full nodes force MORE levels for the same data.\n\nWORKED EXAMPLE -- dense vs sparse index block-access comparison\nA data file has 100,000 records stored 20 records per block (so 5,000 data blocks), sorted on the search key. A dense index entry is 12 bytes; a sparse index entry is also 12 bytes; the index block size is 1024 bytes (so up to floor(1024/12)=85 entries per index block).\n1. step: Dense index needs one entry PER RECORD: 100,000 entries / 85 entries-per-block = ceil(100,000/85) = 1177 index blocks.\n2. step: Sparse index needs one entry PER DATA BLOCK: 5,000 entries / 85 entries-per-block = ceil(5,000/85) = 59 index blocks.\n3. step: Sparse index uses drastically fewer blocks (59 vs 1177) because it only tracks block boundaries, not every record -- but it ONLY works because the data file is sorted on this key, letting the database scan forward within the found block to locate the exact record.\nGATE TRAP: if the SAME 100,000-record file were NOT sorted on this search key (e.g. building a secondary index on a different column), only the DENSE option (1177 blocks) would even be valid -- sparse indexing is not an available choice for an unsorted file, regardless of how much smaller it would be.\n";
})();

(function(){ var t = window.GATE_DATA.questions['dbms'].topics.find(function(t){return t.id==='dbms-transactions';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n• Transaction. A **transaction** is a sequence of one or more database operations (reads and writes) that must be treated as a SINGLE indivisible unit of work -- e.g. transferring money between two bank accounts is one transaction made of a debit and a credit that must both happen, or neither.\n• Why concurrency needs control. When MULTIPLE transactions run at the same time (for performance, since one-at-a-time execution wastes CPU/disk idle time), their individual read/write operations can get INTERLEAVED. Without control, this interleaving can produce a result different from ANY possible one-at-a-time (serial) execution -- corrupting data even though each transaction is individually correct. Concurrency control is the set of rules/protocols that prevent this.\n• ACID. **Atomicity** (all-or-nothing: a transaction's effects are either fully applied or not applied at all), **Consistency** (a transaction takes the database from one valid state to another valid state, preserving all integrity rules), **Isolation** (concurrently running transactions should not see each other's uncommitted, in-progress changes), **Durability** (once a transaction commits, its effects survive even a crash immediately after).\n• Schedule. A **schedule** is one specific chronological interleaving of the operations of several transactions. A **serial schedule** runs transactions one completely after another with no interleaving at all -- always considered correct, since it's exactly equivalent to running them one at a time.\n\nEVERY EDGE CASE\n• Conflicting operations -- the only pairs that matter. Two operations from DIFFERENT transactions CONFLICT if and only if they access the SAME data item AND at least one of them is a WRITE. So: read-read never conflicts (order can be swapped freely); read-write and write-read conflict; write-write conflicts. Only conflicting operations matter for the theory below -- non-conflicting operations can always be reordered/swapped without changing the final effect.\nGATE TRAP: assuming read-write pairs on DIFFERENT data items conflict is wrong -- the SAME data item condition is mandatory; two operations on different items never conflict no matter what they are.\n• Conflict serializability. A schedule is **conflict serializable** if it can be transformed into some SERIAL schedule by repeatedly swapping adjacent NON-conflicting operations. Equivalently (and this is the practical test method), build a **precedence graph**: one node per transaction, and draw an edge Ti -> Tj whenever some operation of Ti conflicts with, and comes BEFORE, some operation of Tj on the same data item. The schedule is conflict serializable IF AND ONLY IF this graph has NO CYCLE.\n1. step: List every pair of conflicting operations (same data item, at least one write) across all transaction pairs.\n2. step: For each conflicting pair, note which transaction's operation occurs EARLIER in the schedule -- draw a directed edge from that earlier transaction to the later one.\n3. step: Merge any duplicate edges between the same pair of nodes (keep just one edge if multiple conflicts point the same direction).\n4. step: Check the resulting graph for a cycle (using DFS or simple visual inspection for small graphs).\n5. step: No cycle => conflict serializable (and any topological sort of the graph gives a valid equivalent serial order). A cycle exists => NOT conflict serializable.\n• View serializability -- a strictly WEAKER (more permissive) notion. A schedule is **view serializable** if it is view-equivalent to some serial schedule, meaning: (a) every transaction reads the same INITIAL values, (b) every 'final write' (the write that survives to the end) is the same transaction in both schedules, and (c) for every read, it reads the value written by the same writer (or the initial value) in both schedules. Every conflict-serializable schedule is automatically view-serializable, but the CONVERSE is NOT true.\nGATE TRAP: schedules containing a **blind write** (a write to a data item without first reading it) are exactly where conflict serializability and view serializability can DIVERGE -- a schedule with blind writes can be view-serializable while its precedence graph has a cycle (making it NOT conflict serializable). If a question explicitly mentions blind writes and asks about serializability, check view-serializability separately rather than relying only on the precedence-graph cycle test.\n• Recoverable, cascadeless, and strict schedules -- three INCREASING levels of safety regarding commits and failures. A schedule is **recoverable** if, whenever a transaction Tj reads a data item previously written by Ti, then Ti must COMMIT before Tj commits (so that if Ti later aborts, Tj -- which read Ti's uncommitted value -- hasn't already committed and can still be undone). A schedule is **cascadeless** (avoids cascading rollback) if every transaction reads ONLY values written by ALREADY-COMMITTED transactions (stronger than recoverable -- Tj must wait for Ti to commit BEFORE reading, not just before Tj itself commits). A schedule is **strict** if every transaction's WRITE is not read OR overwritten by any other transaction until Ti commits or aborts (the strongest of the three, and what most real database systems enforce via strict two-phase locking).\nFORMULA: strict implies cascadeless implies recoverable (in one direction only) -- strict subset-of cascadeless subset-of recoverable subset-of ALL schedules.\nGATE TRAP: a common wrong belief is that these three properties are unrelated categories -- they are actually NESTED: every strict schedule is automatically cascadeless, and every cascadeless schedule is automatically recoverable, but NOT the reverse. A schedule can be recoverable WITHOUT being cascadeless (if a transaction reads an uncommitted value but happens to commit only after the writer commits), and cascadeless WITHOUT being strict.\n• Two-phase locking (2PL) guarantees conflict serializability but NOT automatically recoverability. Basic 2PL only requires that a transaction acquire ALL its locks before releasing ANY lock (growing phase, then shrinking phase) -- this alone guarantees conflict serializability, but a transaction could still release a write-lock and let another transaction read that (possibly later-aborted) value before the first transaction commits, breaking recoverability. **STRICT 2PL** additionally holds ALL locks (especially exclusive/write locks) until the transaction actually COMMITS or ABORTS, which additionally guarantees the schedule is also strict (and therefore cascadeless and recoverable too).\nGATE TRAP: 'uses 2PL' alone does NOT guarantee recoverability -- only STRICT 2PL (or an equivalent stricter protocol) gives you serializability AND recoverability/cascadelessness together; plain 2PL only promises the serializability part.\n• Deadlock is possible under 2PL. Because 2PL allows transactions to request locks incrementally during the growing phase, TWO transactions can each hold a lock the other needs, causing a **deadlock** (circular wait) -- 2PL prevents inconsistency from concurrency but does NOT prevent deadlock; systems need a separate deadlock detection (wait-for graph cycle check) or prevention scheme (e.g. wait-die, wound-wait, using transaction timestamps) layered on top.\n\nWORKED EXAMPLE -- building a precedence graph and finding a cycle\nSchedule S with three transactions, operations occurring in this exact chronological order: R1(A), W2(A), R3(A), R2(B), W1(B), W3(B).\n1. step: Identify all conflicting pairs on data item A: R1(A) before W2(A) -- conflict (T1,T2), edge T1->T2. W2(A) before R3(A) -- conflict (T2,T3), edge T2->T3.\n2. step: Identify all conflicting pairs on data item B: R2(B) before W1(B) -- conflict (T2,T1), edge T2->T1. W1(B) before W3(B) -- conflict (T1,T3), edge T1->T3.\n3. step: Collect all edges found: T1->T2, T2->T3, T2->T1, T1->T3.\n4. step: Check for a cycle: T1->T2 and T2->T1 form a direct 2-node cycle (T1->T2->T1) regardless of the other edges.\n5. step: Since the precedence graph has a cycle, schedule S is NOT conflict serializable.\nGATE TRAP: even though T1->T3 and T2->T3 look like they'd support a nice serial order T1,T2,T3, the SEPARATE edge T2->T1 (from the B conflicts) directly contradicts it -- you must collect edges from EVERY conflicting data item across the WHOLE schedule before concluding serializability; checking only one data item's conflicts and stopping early is a common shortcut mistake that misses cycles.\n\nWORKED EXAMPLE -- classifying recoverable / cascadeless / strict\nSchedule S2: T1 writes A. T2 reads the value of A written by T1 (before T1 commits). T2 commits. T1 commits shortly after.\n1. step: Check recoverability: T2 read a value written by T1; is T1 committed BEFORE T2 commits? Here T2 commits BEFORE T1 commits -- so if T1 were to abort after T2 has already committed, T2's committed result would be based on a value that no longer exists, and T2 cannot be undone (it's already committed) -- this VIOLATES recoverability. S2 is NOT recoverable.\n2. step: Since S2 is not even recoverable, it is automatically also not cascadeless and not strict (both stronger conditions imply recoverability, so failing recoverability fails all three).\nGATE TRAP: a question may FIX the schedule by simply swapping the commit order (T1 commits before T2 commits) -- that single reordering alone makes it recoverable, but it may still NOT be cascadeless, because T2 read T1's value BEFORE T1 had committed at all (cascadelessness requires the read itself to only ever touch already-committed data, not just the eventual commit ORDER) -- fixing recoverability does not automatically fix cascadelessness; check each property independently against its own exact definition.\n\nWORKED EXAMPLE -- strict 2PL avoiding cascading rollback\nTransactions T1 and T2 both need to access data item A, using Strict 2PL.\n1. step: T1 acquires an exclusive lock on A, writes A, and continues its growing phase (acquiring any other locks it needs).\n2. step: T2 requests a lock on A (to read or write it) -- under Strict 2PL, T1's exclusive lock on A is held until T1 COMMITS or ABORTS, so T2's request is BLOCKED and must wait.\n3. step: T1 commits, releasing all its locks at once (Strict 2PL releases everything only at commit/abort, not gradually during a shrinking phase).\n4. step: Only NOW can T2 acquire the lock on A and read T1's committed (never uncommitted) value.\nKEY: because T2 could never see T1's value before T1 committed, there is no possibility of a cascading rollback if T1 had aborted instead -- this is exactly why Strict 2PL guarantees both conflict serializability (from the 2PL growing/shrinking rule) and strictness (from holding locks until commit/abort) simultaneously, and is the concurrency control protocol most real relational databases actually implement.\n";
})();
