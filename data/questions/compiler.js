...
window.GATE_DATA.questions['compiler'].topics.find(function(t){return t.id==='compiler-parsing';}).theory.deep = "PARSER RESPONSIBILITIES AND PHASE INPUT/OUTPUT\n\n• Input: the token stream produced by the lexical analyzer (each token carries a type and optional attribute value, e.g. a symbol-table pointer for id).\n• Output: a parse tree (concrete syntax tree) or, in practice, an abstract syntax tree (AST) plus a decision of accept/reject, and a call sequence of semantic actions if the parser is syntax-directed.\n• The parser verifies that the token stream is a sentence of the context-free grammar (CFG) describing the language's syntax; it reports syntax errors when it is not, and in most GATE-relevant designs performs error recovery (panic mode, phrase level) to keep compiling.\n• Two families: top-down (build the tree root to leaves, predict productions, e.g. recursive descent, LL(1)) and bottom-up (build leaves to root, reduce handles, e.g. operator-precedence, LR family: SLR(1), CLR(1)/LR(1), LALR(1)).\n• A CFG must first be made suitable for the chosen technique: left recursion must be eliminated for top-down parsing (direct: A -> Aα | β becomes A -> βA', A' -> αA' | ε; indirect recursion is removed by substitution first), and common prefixes must be removed by left factoring (A -> αβ1 | αβ2 becomes A -> αA', A' -> β1 | β2) so a single lookahead symbol suffices to choose a production.\n\nFIRST AND FOLLOW COMPUTATION RULES (CHECKLIST)\n\nFIRST(α) = the set of terminals that can begin some string derived from α; ε ∈ FIRST(α) if α can derive ε.\n• Rule 1: If X is a terminal, FIRST(X) = {X}.\n• Rule 2: If X -> ε is a production, add ε to FIRST(X).\n• Rule 3: If X is a nonterminal and X -> Y1 Y2 ... Yk, then for each i from 1 to k: add FIRST(Yi) − {ε} to FIRST(X); stop adding further Yi's contributions as soon as some Yj does NOT have ε in its FIRST set. If ε ∈ FIRST(Yi) for all i = 1..k (i.e. the whole right side can vanish), add ε to FIRST(X) too.\n• Compute FIRST for a string X1X2...Xn (needed while building FIRST of a production's body) exactly as rule 3 describes: scan left to right, accumulate FIRST of each symbol minus ε, stop at the first symbol whose FIRST does not contain ε.\n• Iterate rules over all productions until no set changes (fixed point); for non-recursive grammars one left-to-right, bottom-up pass over the dependency order suffices.\n\nFOLLOW(A) = the set of terminals that can appear immediately to the right of A in some sentential form; $ (end marker) is in FOLLOW(S) for the start symbol S.\n• Rule 1: Place $ in FOLLOW(S), S the start symbol.\n• Rule 2: If there is a production A -> αBβ, then everything in FIRST(β) except ε is in FOLLOW(B).\n• Rule 3: If there is a production A -> αB, or a production A -> αBβ where ε ∈ FIRST(β) (β vanishes), then everything in FOLLOW(A) is in FOLLOW(B).\n• Apply rules 2 and 3 to EVERY occurrence of every nonterminal on the right-hand side of every production, and iterate to a fixed point (FOLLOW sets can depend on each other, unlike a single top-down FIRST pass).\n• FOLLOW is defined only for nonterminals; never compute FOLLOW of a terminal.\n\nLL(1) PARSING TABLE CONSTRUCTION\n\nFor each production A -> α:\n• For each terminal a ∈ FIRST(α), add A -> α to M[A, a].\n• If ε ∈ FIRST(α), then for each terminal b ∈ FOLLOW(A), add A -> α to M[A, b]; if ε ∈ FIRST(α) and $ ∈ FOLLOW(A), also add A -> α to M[A, $].\nA grammar is LL(1) exactly when this procedure never places two different productions in the same table cell. The two conflict conditions to check explicitly are: (a) two productions A -> α | β with FIRST(α) ∩ FIRST(β) ≠ ∅ (this alone kills LL(1) even before touching FOLLOW), and (b) if ε ∈ FIRST(α) for some alternative, then FIRST(β) for every other alternative must be disjoint from FOLLOW(A) as well. Left recursion (direct or indirect) and common left prefixes always cause conflicts and must be removed before table construction. A predictive (LL(1)) parser is a stack + table driven automaton: push $S; repeatedly compare top-of-stack with lookahead — terminal match pops both and advances input, nonterminal on top consults M[nonterminal, lookahead] to choose the production replacing it (pushed in reverse order), and error is reported on a blank table entry.\n\nLR PARSING: ITEMS, CLOSURE, GOTO\n\nAn LR(0) item is a production with a dot marking how much of the body has been recognized, e.g. A -> α·β. LR(1) items additionally carry a lookahead terminal, written [A -> α·β, a].\n• closure(I): start with I; while some item A -> α·Bβ is in the set and B -> γ is a production, add B -> ·γ (LR(0)) — or, for LR(1), add [B -> ·γ, b] for every b ∈ FIRST(βa) where a is the lookahead of the originating item — repeat until no new items are added.\n• goto(I, X): the closure of the set of items [A -> αX·β, a] for every [A -> α·Xβ, a] in I; this is the transition on grammar symbol X.\n• Canonical collection construction: begin with C = {closure({[S' -> ·S, $]})} for the augmented grammar S' -> S; repeatedly compute goto(I, X) for every I ∈ C and every symbol X, adding any new item set to C, until no new sets appear. This canonical collection is exactly the state set of the LR automaton (its transition diagram is the DFA the parser drives on).\n• Action/goto table entries: a shift action shift j is placed for [A -> α·aβ, ...] on terminal a going to state j = goto(I,a); a reduce action reduce A -> α is placed on the lookahead terminals valid for a completed item [A -> α·, a] (SLR uses FOLLOW(A) here instead of the item's own lookahead); accept is placed for [S' -> S·, $].\n\nSLR vs CLR vs LALR — CONSTRUCTION DIFFERENCES AND POWER\n\nGrammar-Class Power Hierarchy: LL(1) ⊂ SLR(1) ⊂ LALR(1) ⊂ LR(1) ⊂ unambiguous CFGs (each inclusion strict).\n\n• SLR(1) (Simple LR): builds the LR(0) canonical collection (no lookahead carried in items) and, for a reduce item A -> α· in state I, places reduce A -> α on every a ∈ FOLLOW(A). Cheapest to build, but FOLLOW(A) is context-insensitive — it merges the \"what can follow A anywhere\" information, which is sometimes too coarse and creates spurious reduce/reduce or shift/reduce conflicts that a context-sensitive method would not have.\n• CLR(1) (Canonical LR): builds LR(1) items directly, so each reduce item's lookahead is exactly the terminals that can follow THIS occurrence of A in THIS context. Most powerful and conflict-free for the largest class of grammars, but the canonical collection can have far more states than the LR(0) collection (state explosion), making parsing tables large.\n• LALR(1) (Look-Ahead LR): builds the full LR(1) canonical collection, then merges LR(1) states that have the same \"core\" (identical LR(0) items, differing only in lookahead sets) into one state, unioning the lookaheads. This gives the same number of states as SLR/LR(0) (compact, like SLR) while using context-sensitive lookaheads (nearly as powerful as CLR). The only way LALR can go wrong relative to CLR is by introducing a reduce/reduce conflict that did not exist before merging (merging never introduces a new shift/reduce conflict, only possibly new reduce/reduce ones).\n\nWitness grammars: the classic separating example is S -> Aa | bAc | dc | bda ; A -> d, which is LALR(1) but not SLR(1) (an SLR(1) construction gets a shift/reduce or reduce/reduce ambiguity via a bloated FOLLOW(A) merging contexts that LALR's precise lookaheads keep apart). A textbook example separating LR(1) from LALR(1) is a grammar such as S -> aAd | bBd | aBe | bAe ; A -> c ; B -> c, where the LR(1) canonical collection keeps two states with core A -> c· / B -> c· apart with disjoint lookaheads {d} and {e} in different contexts but merges them into one LALR state with lookahead {d, e} on both items, creating a reduce/reduce conflict that CLR(1) never had. This demonstrates concretely why LALR(1) is strictly weaker than CLR(1) even though it is the most common real-world choice (used by yacc/bison) because of its SLR-sized tables.\n\nOPERATOR PRECEDENCE PARSING (BASICS)\n\n• Applies only to \"operator grammars\": no production has an empty right side (no ε-productions) and no two adjacent nonterminals ever appear together on a right side (every pair of adjacent grammar symbols on a body has at least one terminal between/among them).\n• Define three precedence relations between terminal pairs a, b: a ⋖ b (a yields precedence to b), a ≐ b (a has the same precedence as b, used for matching pairs like ( and )), a ⋗ b (a takes precedence over b) — built from operator precedence and associativity rules (e.g. * ⋗ + on the left because * binds tighter, and ⋖ on the right side of a lower one).\n• Parsing uses a stack of terminals (nonterminals are skipped over) and the input; at each step compare the topmost stack terminal with the next input terminal: if ⋖ or ≐, shift; if ⋗, reduce (pop until you can identify and reduce a handle back to a nonterminal placeholder); mismatch means error.\n• Operator precedence parsers are simple and fast but cannot handle all CFGs, cannot report as many kinds of syntactic detail as a full LR parser, and are considered a fairly weak, largely historical technique next to full LR methods — GATE mainly tests recognizing precedence relation tables and identifying shift versus reduce decisions from them.\n\nATTRIBUTE GRAMMAR EVALUATION ORDERS\n\n• S-attributed grammars use only synthesized attributes (computed bottom-up, from an attribute's children); they fit naturally with bottom-up (LR) parsing since semantic actions run at reduction time using values already sitting below on the parser's semantic stack.\n• L-attributed grammars allow synthesized attributes and inherited attributes so long as each inherited attribute of a symbol on a production's right side depends only on the attributes of symbols to its LEFT in that same production (plus inherited attributes of the head); L-attributed grammars can be evaluated with a single left-to-right depth-first traversal, and this is exactly the class that fits top-down (predictive) parsing with embedded actions, and is also handled by bottom-up parsers using a translation to remove leftover inherited dependencies.\n• General attribute grammars (arbitrary dependency graphs) need a topological-sort based evaluator over the dependency graph built per parse tree, and are NOT guaranteed to admit a simple single-pass evaluation order; a dependency-graph cycle makes the attribute grammar ill-formed (unevaluable).\n\nWORKED EXAMPLE 1 — FULL FIRST/FOLLOW FOR A 4-PRODUCTION GRAMMAR\n\nGrammar: (1) S -> A B (2) A -> a A (3) A -> ε (4) B -> b B | ε — treat (4) as two productions for a clean 4+1 count, but let's use exactly these 4 productions with B -> b B taken as production 4 and B -> ε folded as a listed alternative:\nS -> A B\nA -> a A | ε\nB -> b B | ε\n\nFIRST computation: FIRST(a) = {a}, FIRST(b) = {b} (terminals, rule 1).\nFIRST(A): from A -> aA, add FIRST(a) = {a}; a does not have ε so stop for this alternative → contributes {a}. From A -> ε, add ε directly. So FIRST(A) = {a, ε}.\nFIRST(B): symmetric to A → FIRST(B) = {b, ε}.\nFIRST(S): from S -> AB, first take FIRST(A) − {ε} = {a}; since ε ∈ FIRST(A), continue to FIRST(B) − {ε} = {b}; since ε ∈ FIRST(B) too, and we have exhausted the body (no more symbols), add ε as well. So FIRST(S) = {a, b, ε}.\n\nFOLLOW computation: FOLLOW(S) = {$} by rule 1 (start symbol).\nFOLLOW(A): A appears in S -> A B — apply rule 2, everything in FIRST(B) − {ε} goes into FOLLOW(A): that is {b}. Since ε ∈ FIRST(B) (B can vanish), rule 3 also applies: FOLLOW(S) ⊆ FOLLOW(A), adding {$}. A also appears in A -> a A (A at the end of the body) — rule 3 applies directly: FOLLOW(A) ⊆ FOLLOW(A), which adds nothing new. So FOLLOW(A) = {b, $}.\nFOLLOW(B): B appears in S -> A B, at the end of the body — rule 3: FOLLOW(S) ⊆ FOLLOW(B), adding {$}. B appears in B -> b B, at the end — rule 3: FOLLOW(B) ⊆ FOLLOW(B), nothing new. So FOLLOW(B) = {$}.\n\nFinal sets: FIRST(S) = {a, b, ε}, FIRST(A) = {a, ε}, FIRST(B) = {b, ε}; FOLLOW(S) = {$}, FOLLOW(A) = {b, $}, FOLLOW(B) = {$}.\nLL(1) table cells built from these: M[A,a] = A->aA (from a ∈ FIRST(aA)); M[A,b] = A->ε and M[A,$] = A->ε (since ε ∈ FIRST(A) and FOLLOW(A) = {b,$}); M[B,b] = B->bB; M[B,$] = B->ε. No cell gets two entries, so this grammar is LL(1).\n\nWORKED EXAMPLE 2 — LR(0) CANONICAL COLLECTION AND SLR CHECK\n\nAugmented grammar: S' -> S ; S -> ( S ) | a.\nI0 = closure({[S' -> ·S, $]}) = { [S'->·S,$], [S->·(S),$], [S->·a,$] } (closure adds S's productions since dot is before S; then since S->·(S) has dot before terminal (, nothing further to expand — operator symbols don't trigger closure).\ngoto(I0, S) = closure({[S'->S·,$]}) = I1 = { [S'->S·,$] } — this is the ACCEPT item (dot at end of S'->S with lookahead $).\ngoto(I0, a) = closure({[S->a·,$]}) = I2 = { [S->a·,$] } — reduce state.\ngoto(I0, \"(\") = closure({[S->(·S),$]}) = I3 = { [S->(·S),$], [S->·(S),$], [S->·a,$] } (dot before S triggers re-adding S's own productions, since the item is now A -> (·S) with lookahead the same as before propagating through closure rules — using $ here as the placeholder single-lookahead grammar for illustration).\ngoto(I3, S) = closure({[S->(S·),$]}) = I4 = { [S->(S·),$] } (dot now before the literal \")\" terminal remaining).\ngoto(I3, \"(\") = I3 (self-loop, same closure as before), goto(I3, a) = I2 (same reduce state as before, shared).\ngoto(I4, \")\") = closure({[S->(S)·,$]}) = I5 = { [S->(S)·,$] } — reduce state for S->(S).\nState count: {I0, I1, I2, I3, I4, I5} — six states total. Since every state has at most one action per lookahead symbol (I2 reduces S->a only on FOLLOW(S) = {$, )}, I5 reduces S->(S) only on FOLLOW(S), and there is no terminal on which both a shift and a reduce are legal in the same state), this grammar is conflict-free under SLR(1); it is SLR(1), and hence also LALR(1) and LR(1) by the hierarchy.\n\nGATE TRAPS\n\n• Forgetting to check ε in EVERY alternative's FIRST when computing FIRST of a body — stopping the scan too early (or too late) is the single most common FIRST-set mistake.\n• Applying FOLLOW rule 2 while forgetting that when β itself is nullable (ε ∈ FIRST(β)), rule 3 must ALSO fire — students frequently apply only rule 2 and lose the extra FOLLOW(A) contribution.\n• Confusing \"grammar is ambiguous\" with \"grammar is not LL(1)\" — an ambiguous grammar is never LL(1)/LR(1)/etc., but a grammar can be unambiguous and still fail LL(1) purely due to left recursion or a FIRST/FOLLOW clash (left factoring or grammar transformation may still rescue it).\n• Believing every SLR(1) conflict means the grammar is inherently ambiguous — it may simply need the sharper lookaheads of CLR(1)/LALR(1), which is exactly why the power hierarchy question type (name a grammar that is LALR but not SLR) recurs on GATE.\n• Mixing up shift/reduce (usually resolved in real tools by preferring shift, e.g. dangling-else) with reduce/reduce conflicts (never automatically resolved sensibly; always signals grammar or table-construction weakness).\n• Assuming LALR(1) always equals CLR(1) in power — LALR merging can create NEW reduce/reduce conflicts (never new shift/reduce ones) that CLR(1) tables never had; memorize this asymmetry.\n• Forgetting operator precedence parsing requires an operator grammar (no adjacent nonterminals, no ε-productions) before precedence relations even make sense.\n• Treating S-attributed as a special, stricter case of L-attributed (true) but then assuming L-attributed grammars need bottom-up-only or top-down-only parsers — they are the class compatible with a single left-to-right depth-first traversal, usable by both parsing families with appropriate translation.";
