...
Printing after forks. If a printf follows n forks, it executes once per process: 2^n times. If the printf is between forks, the naive count is "how many processes exist at that point" — but since printf("A") has no newline, the buffering rule below applies too: each already-buffered "A" gets duplicated by every fork that happens after it, not just executed once per process that reaches the printf call site. In fork(); printf("A"); fork();, the printf call itself runs in 2 processes (giving 2 buffered "A"s), but the second fork() then duplicates EACH of those 2 buffers, so "A" ends up printed 4 times — matching the 4 total processes, not the 2 processes that actually executed the printf statement.
...
ENUMERATING INTERLEAVINGS: FINDING THE ACHIEVABLE RANGE OF A RACE

The shared-memory warning above ("the reader may see half-updated data") can be made precise by actually enumerating every possible instruction-level interleaving of two racing operations, rather than only asserting informally that the result is "unpredictable." This is the SAME technique the synchronisation chapter's own Race Condition material develops in full, restated here in enough detail to work a self-contained problem directly.

1. BREAK each high-level operation (such as counter++) into its actual machine-level steps — a non-atomic increment is really three separate steps: LOAD the current value into a register, INCREMENT the register, STORE the register back to memory.
2. ENUMERATE the distinct orderings in which two (or more) threads' own step sequences can interleave, respecting each individual thread's OWN internal step order (thread A's own LOAD must precede its own INCREMENT, which must precede its own STORE — only the relative INTERLEAVING between different threads' steps is free to vary).
3. FOR each valid interleaving, trace through the actual register and memory values step by step, and record the FINAL value produced.
4. THE ACHIEVABLE RANGE of final values is the SET of distinct results produced across every enumerated interleaving — the MINIMUM possible final value corresponds to the WORST-case interleaving (the most updates get "lost" by being overwritten before being seen), and the MAXIMUM corresponds to the interleaving where no updates are lost at all (equivalent to fully serialised, non-overlapping execution).

1. Worked example (interleaving enumeration for a shared counter, independently verified): a shared counter starts at 0. Two threads, T1 and T2, EACH execute counter++ TWICE (non-atomic: LOAD, INCREMENT, STORE per increment), with arbitrary interleaving allowed between the two threads. Find the MINIMUM possible final value of counter. The absolute WORST case: T1 LOADs 0 (register_T1=0), T2 LOADs 0 (register_T2=0) — both see the SAME stale value before either stores back. T1 INCREMENTs to 1, STOREs 1 (counter=1). T2 INCREMENTs to 1, STOREs 1 (counter=1, OVERWRITING T1's update with an equal but independently-computed value). Now for the SECOND increment on each thread: T1 LOADs 1, T2 LOADs 1 (both see the same value again), T1 INCREMENTs to 2, STOREs 2, T2 INCREMENTs to 2, STOREs 2 — final counter = 2, NOT 4 (which is what four increments would give if none were lost). This confirms the achievable minimum is 2 (each pair of concurrent increments loses one update to overwriting), while the maximum achievable (fully serialised, no interleaving loss) is 4 — the TRUE achievable range for this specific scenario is {2, 3, 4}, not a single deterministic value.

ROUND ROBIN MECHANICS: TIME-SLICE DISPATCH COUNTING

Round Robin (introduced by name here, with its full scheduling-policy treatment in the next chapter) can still be used directly in a total-time computation once its ONE governing mechanical rule is known: each process, whenever it is dispatched, runs for AT MOST one time QUANTUM before being preempted and cycled to the back of the ready queue, repeating this cycle until the process's own remaining burst time reaches zero.

1. THE NUMBER of separate DISPATCHES (times a process is given the CPU) a process with burst time B needs, under a quantum of size Q, is ⌈B/Q⌉ — the process runs Q time units on every dispatch except possibly its LAST, which runs only the remaining leftover (B minus every earlier full quantum already consumed).
2. IF a fixed CONTEXT-SWITCH OVERHEAD is charged every time the CPU switches to a DIFFERENT process (a common simplifying assumption in numeric problems), the total wall-clock time for a set of processes cycling through Round Robin is the SUM of every process's own actual burst time, PLUS one context-switch overhead charged for EVERY dispatch across ALL processes (or, in some conventions, for every dispatch EXCEPT the very first, since no "switch" is needed to start the very first process from an idle CPU) — always confirm which convention a specific question intends before counting switches.

1. Worked example (Round Robin dispatch count and total time, independently verified): 5 processes, each with a burst time of 18 units, quantum = 4, and each context switch costs 1 unit of overhead. Find the total wall-clock time to run all 5 processes to completion (counting a switch before every dispatch, including the very first). Dispatches needed per process = ⌈18/4⌉ = 5 (four dispatches of 4 units, one final dispatch of 2 units: 4+4+4+4+2=18, confirmed). Total dispatches across all 5 processes = 5 processes × 5 dispatches each = 25 dispatches, hence 25 context switches (one per dispatch, by this convention). Total burst time across all processes = 5×18 = 90 units. Total wall-clock time = 90 (actual work) + 25×1 (switch overhead) = 90+25 = 115 units.
