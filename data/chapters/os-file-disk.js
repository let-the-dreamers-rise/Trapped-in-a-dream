...
An unexpected crash or power loss MID-WAY through a multi-step file-system update (say, updating an inode's block pointers AND the free-space bitmap for a new allocation) can leave the on-disk structures in an INCONSISTENT state — some of the update applied, the rest not — a genuinely different failure mode from an ordinary application crash, since the FILE SYSTEM's own metadata itself is now corrupted, not just one process's in-memory state.

1. A JOURNALING FILE SYSTEM protects against this by writing a description of an intended multi-step update to a separate, append-only JOURNAL (or LOG) area on disk FIRST, before applying any of those steps to the actual file-system structures themselves — this is the SAME "write intent first, then apply" write-ahead-logging discipline this book's own Database Transactions chapter developed in full for exactly the analogous problem of crash-safe multi-step updates.
2. IF a crash occurs BEFORE the journal entry is fully written, the intended update simply never happened at all (as far as recovery is concerned) — the file system remains in its PREVIOUS, consistent state, with nothing to redo.
3. IF a crash occurs AFTER the journal entry is fully written but BEFORE (or partway through) actually applying it to the real file-system structures, RECOVERY simply REPLAYS the completed journal entries against the file system on the next mount, finishing whatever the crash interrupted — since the entire intended update was already durably recorded, replaying it produces the SAME final, consistent state as if the crash had never interrupted it.
4. AN OLDER, non-journaling alternative is FSCK (file system consistency check): a special recovery TOOL that scans the ENTIRE file system after an unclean shutdown, cross-checking every inode, block pointer, and free-space structure against every other for consistency, and repairing whatever it finds broken — this is CORRECT but dramatically SLOWER than journal replay, since fsck must examine the WHOLE file system (proportional to total disk size) rather than only the small set of updates that were actually in flight at crash time (proportional to journal size).

GATE TRAP: assuming fsck and journaling solve the SAME problem in the SAME way, differing only in speed, misses the more fundamental distinction — fsck DETECTS and REPAIRS inconsistency AFTER THE FACT by scanning everything, with no record of what was actually being attempted at crash time, while journaling PREVENTS the file system from ever being left in a genuinely unrecoverable state by recording INTENT beforehand, allowing a targeted, fast REPLAY rather than a full, uninformed scan; a question describing a system that must recover QUICKLY after crashing, proportional only to the SIZE of the interrupted operation rather than the WHOLE disk, is describing journaling specifically, not fsck.

SSD WEAR LEVELING: A FLASH-SPECIFIC CONSTRAINT

Solid-state drives (SSDs) store data in FLASH memory cells, which — unlike a magnetic disk's platters — have a GENUINE, FINITE limit on how many times any single cell can be ERASED and rewritten before it wears out and becomes unreliable, a constraint with NO analogue anywhere in this chapter's earlier magnetic-disk material.

1. FLASH memory can be WRITTEN in small units (pages) but must be ERASED in much LARGER units (blocks, made up of many pages) — and each erase cycle GENUINELY degrades the physical cell slightly, with a MANUFACTURER-RATED maximum number of erase cycles (a fixed endurance limit) before the cell becomes unreliable.
2. IF the SAME small region of the drive were repeatedly overwritten far more often than the REST of the drive (as an ordinary, naive "write in place" file system might naturally do for frequently-updated metadata), that SPECIFIC region would wear out and fail LONG before the rest of the drive's capacity has been meaningfully used at all — a highly UNEVEN, wasteful failure pattern.
3. WEAR LEVELING is the SSD controller's own remapping technique that ensures write/erase cycles are spread as EVENLY as possible across the ENTIRE physical flash array, REGARDLESS of which LOGICAL block address the operating system or file system believes it is writing to — the controller maintains its OWN internal mapping from logical to physical addresses (analogous in spirit to virtual-to-physical page translation), and can freely relocate data physically to balance wear, entirely transparent to the file system layered on top.
4. THIS transparent remapping is ALSO why SSDs benefit from the TRIM command: when a file system deletes a file, it normally only marks the LOGICAL blocks as free in its own metadata, with no direct way to tell the SSD's controller that the underlying PHYSICAL flash pages are now genuinely free — TRIM explicitly informs the SSD which logical blocks are no longer in use, letting the wear-leveling controller include them in its own erase-and-reuse pool rather than treating them as still-live data that must be preserved.

GATE TRAP: assuming an SSD's wear-leveling remapping is simply "like a disk's own bad-sector remapping" understates a genuinely different underlying cause — bad-sector remapping on a magnetic disk responds to actual PHYSICAL DEFECTS discovered on specific sectors, while SSD wear leveling PROACTIVELY spreads ordinary, otherwise-healthy write/erase load evenly across the WHOLE device specifically to prevent any one region from reaching its finite erase-cycle limit prematurely — one is reactive defect-avoidance, the other is proactive load-balancing against a fundamentally different (finite-endurance) physical constraint that magnetic disks simply do not have.
