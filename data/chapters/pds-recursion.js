...
6. Write the recurrence for, and solve, the running time of this function, called on an array of size n:
   ...
   Each call does O(n) work (the call to process) and makes one recursive call on n − 1, so T(n) = T(n − 1) + n, with T(1) = O(1) (call it 0 for the base, since a size-1 array needs no processing — the base case here is T(1), not T(0), since the code only recurses down to n=1 before stopping). This is a close cousin of the second standard form derived earlier, but with the base case shifted by one: unrolling T(n) = T(n−1) + n = T(n−2) + (n−1) + n = … = T(1) + (2 + 3 + … + n) = 0 + (2+3+…+n) gives the closed form T(n) = n(n+1)/2 − 1 (the sum from 2 to n, one short of the sum from 1 to n), so the algorithm still runs in O(n^2) time — quadratic, because it repeats a linear scan once for every one of the n−1 levels of recursion that actually do work, with the exact constant term differing from the T(0)-based form purely because of where this specific recursion's base case sits.
