...
THE JOSEPHUS RECURRENCE — because GATE rarely asks you to actually simulate the circular list by hand for large n; instead it asks for the position of the survivor directly, and the simulation above is far too slow to do that in your head. Number the n people 0 through n − 1 (0-INDEXED — this convention is what makes the recurrence clean) and let J(n) be the 0-indexed position of the survivor when every k-th person is eliminated. The key observation: after the FIRST elimination (the person at 0-indexed position k − 1 is removed), the remaining n − 1 people form a fresh circle, and counting continues from the person immediately after the one just removed — call that person the new "position 0" of a smaller (n − 1)-person Josephus problem. If J(n − 1) is the survivor's 0-indexed position IN THAT SMALLER PROBLEM's own numbering, then in the ORIGINAL numbering the survivor sits k positions after the eliminated one, wrapped around the (still n-person-wide) circle:
    J(1) = 0                              (base case: one person left, trivially the survivor, at position 0)
    J(n) = (J(n − 1) + k) mod n,   n > 1
This recurrence is built bottom-up — compute J(1) = 0, then J(2), J(3), ... up to J(n) — each step doing one addition and one mod, which is why it beats simulation: O(n) arithmetic operations instead of O(n) list-pointer walks of shrinking circles (the simulation is also O(n) total work, but the recurrence needs no data structure at all, just two integers, and is far faster to execute by hand under exam pressure).
WORKED EXAMPLE: n = 5 people (0,1,2,3,4), k = 2 (every 2nd person eliminated). J(1) = 0. J(2) = (J(1) + 2) mod 2 = (0 + 2) mod 2 = 0. J(3) = (J(2) + 2) mod 3 = (0 + 2) mod 3 = 2. J(4) = (J(3) + 2) mod 4 = (2 + 2) mod 4 = 0. J(5) = (J(4) + 2) mod 5 = (0 + 2) mod 5 = 2. So with 5 people 0-indexed, the survivor is at position 2 (the person originally labelled "2").
GATE TRAP: if the problem states people are numbered starting from 1 rather than 0, solve the recurrence in 0-indexed terms exactly as above and add 1 to J(n) only at the very last step — do NOT shift k or the mod base, only the final answer's labelling. A common error is to try to "fix" the recurrence itself for 1-indexing, which breaks the mod arithmetic.
...
THE XOR LINKED LIST — MEMORY-EFFICIENT DOUBLY LINKED LISTS

A plain doubly linked list pays for O(1) predecessor access with a second pointer field in every node (prev, alongside next). An XOR LINKED LIST gets the same "reach either neighbour" capability while storing only ONE address-sized field per node, called npx (for "next XOR prev"), defined as:
    npx = address(prev) XOR address(next)
using the bitwise XOR of the two neighbouring nodes' memory addresses (for the first node, whose prev is conceptually NULL i.e. address 0, npx = 0 XOR address(next) = address(next); symmetrically for the last node).

The trick that makes this usable is the self-inverse property of XOR: for any addresses a and b, (a XOR b) XOR a = b, and (a XOR b) XOR b = a. So if you are standing AT a node and you arrived from the PREVIOUS node (whose address you still have, call it prevAddr), you can recover the NEXT node's address with a single XOR:
    nextAddr = node->npx XOR prevAddr
because node->npx already equals prevAddr XOR nextAddr, so XORing it again with prevAddr cancels prevAddr out and leaves nextAddr. Traversal in EITHER direction is therefore possible, but only if you always carry along "the address of the node you just came from" as you walk — a plain single next pointer is not enough on its own; you need the pair (current node, previous node) at every step, exactly analogous to how the Josephus recurrence above needed to carry the previous survivor's position forward.

KEY: The entire benefit of an XOR linked list is memory, not speed — it does not turn any O(n) doubly linked list operation into something faster than O(n); insertion, deletion and traversal all remain the same asymptotic complexity as an ordinary DLL. What is saved is exactly one pointer field's worth of memory PER NODE, which matters only when n is large and memory-per-node genuinely constrains the design (this is why GATE questions on XOR lists are almost always about the npx arithmetic itself, or about the traversal requiring two addresses to be carried, rather than about complexity).

GATE TRAP: A frequently tested false claim is "an XOR linked list node stores the XOR of the two neighbouring NODES" — it stores the XOR of their ADDRESSES (pointer values), not the XOR of the nodes' data or of the node structures themselves. A second common trap: you cannot recover next OR prev from npx alone without already knowing one of the two addresses (typically supplied by the traversal direction you are walking in) — npx by itself, with no other information, is not invertible to either individual address.
