...
4. Checking essentiality: m2 and m10 are covered ONLY by B′D′ (no other group reaches them), so B′D′ is essential. m7 and m15 are covered ONLY by BD, so BD is essential. Checking the remaining minterms 0,1,8,9,5,13 against just B′C′ and C′D: m0,m8 need B′C′ (not covered by C′D, since C′D requires D=1 but m0,m8 have D=0), so B′C′ is ALSO essential; m5,m13 need C′D (not covered by B′C′, since B′C′ requires C=0 but m5,m13 have C=1), so C′D is essential too — all four groups turn out to be essential.
5. Minimal SOP: f = B′C′ + C′D + BD + B′D′ — 4 terms, 8 literals total, and this is genuinely minimal since every one of the four groups is essential (none can be dropped without leaving some minterm uncovered).
...
9. QUINE-McCLUSKEY. Find the prime implicants of f = Σm(0,1,2,3) over 2 variables (A,B), using Quine-McCluskey. Binary: 0=00,1=01,2=10,3=11. Weight groups: weight 0: {00}. weight 1: {01,10}. weight 2: {11}. Combining weight 0 with weight 1: 00 vs 01 differ in last bit → 0− (A′, covering 0,1). 00 vs 10 differ in first bit → −0 (B′, covering 0,2). Combining weight 1 with weight 2: 01 vs 11 differ in first bit → −1 (B, covering 1,3). 10 vs 11 differ in last bit → 1− (A, covering 2,3). No further combination is possible among {0−,−0,−1,1−} (each already uses its one dash). All four are prime implicants, but checking essentiality: minterm 0 is covered by BOTH A′ and B′ (not uniquely essential to either alone), and similarly no single minterm is covered by only one PI here — this is the "checkerboard-adjacent" case needing the residual set-cover step: choosing any 2 of the 4 PIs that together cover all of {0,1,2,3} (for instance A′+A, or B′+B) gives a valid minimal 2-term cover, and indeed f=Σm(0,1,2,3) is simply the constant function 1 (every input gives output 1), consistent with A′+A=1 as the simplified answer.
...
14. PRESERVED-PROPERTY COMPLETENESS CHECK. Using Post's completeness criterion, determine whether {AND, XOR} is functionally complete. Check each of the five preserved classes: T0 (preserves 0)? AND(0,0)=0 and XOR(0,0)=0, so both preserve 0 — the set stays inside T0, meaning it CANNOT produce a function violating T0 (such as a constant-1 function), yet functional completeness only requires building AND, OR, NOT, not every conceivable function — checking further: is the set entirely monotone? XOR is NOT monotone (increasing one input from 0 to 1 while the other stays 1 takes XOR from 1 to 0), so the set escapes the monotone trap that doomed {AND,OR}. Is the set entirely affine? AND itself is not affine, so the set escapes the affine trap that doomed {XOR} alone. Checking self-dual and T1 similarly shows the set is not confined to any of the five classes entirely — by Post's criterion, {AND, XOR} IS functionally complete (and indeed, NOT(x)=XOR(x,1) and OR(x,y)=XOR(XOR(x,y),AND(x,y)) can both be constructed directly, confirming the criterion's verdict by explicit construction). This double-checking pattern — a preserved-property argument followed by an explicit construction confirming the same conclusion — is worth applying whenever time allows, since the abstract argument alone can occasionally be misapplied to the wrong class, while an explicit construction settles the question beyond doubt, leaving no ambiguity about whether every one of the five preserved classes was actually checked correctly.
