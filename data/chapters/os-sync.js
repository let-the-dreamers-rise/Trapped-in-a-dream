...
RESOURCE-ALLOCATION GRAPHS: SINGLE VS MULTI-INSTANCE CYCLES

A brief, self-contained summary of a distinction developed FULLY in the next chapter's own Deadlock material, since it is worth stating here wherever a synchronisation question already draws a resource-allocation-style diagram: a CYCLE in a Resource-Allocation Graph (RAG) means something DIFFERENT depending on whether each resource TYPE has exactly one instance or several.

1. IF every resource type in the graph has EXACTLY ONE instance, a cycle in the RAG is BOTH necessary AND sufficient for deadlock — a cycle GUARANTEES a genuine deadlock, no further check is needed.
2. IF some resource type has MULTIPLE instances, a cycle is necessary but NOT sufficient — a cycle MAY exist without an actual deadlock, since a process WAITING on one instance of a multi-instance resource might still be satisfied by a DIFFERENT instance of the same type becoming free, even while the cycle itself persists in the graph.
3. FOR the multi-instance case specifically, resolving whether a cycle actually indicates deadlock requires a BANKER'S-ALGORITHM-STYLE safety check: treat the current allocation and outstanding requests as a snapshot, and check whether SOME ordering of the remaining processes could still each be satisfied and finish, exactly as the Banker's safety algorithm (covered in full in the Deadlock chapter) does for a general system, rather than relying on the graph shape alone.

GATE TRAP: assuming a RAG cycle ALWAYS means deadlock, without first checking whether every resource type involved has exactly one instance, is a common and serious error — a cycle among MULTI-instance resources requires the additional Banker's-style safety check before concluding deadlock is actually present; only in the SINGLE-instance-per-type case does the cycle alone settle the question.
