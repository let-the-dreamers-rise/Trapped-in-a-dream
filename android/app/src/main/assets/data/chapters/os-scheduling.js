...
MONITORS AND CONDITION VARIABLES: A SELF-CONTAINED SUMMARY

A MONITOR is a higher-level synchronisation construct than a semaphore — a module whose own procedures are AUTOMATICALLY mutually exclusive (at most one process executes any procedure of a given monitor at a time), enforced by the language/runtime itself rather than hand-coded by the programmer.

1. A CONDITION VARIABLE x, declared inside a monitor, lets a process WAIT for some condition to become true without holding up every OTHER process forever: x.wait() suspends the caller and releases the monitor's own mutual exclusion so someone else can enter; x.signal() wakes ONE process waiting on x, if any, and does nothing (is simply discarded) if no process is currently waiting — unlike a semaphore's signal(), a monitor's signal() does NOT "remember" itself for a future waiter.
2. THE SIGNAL SEMANTICS split into two disciplines: HOARE monitors (signal-and-wait) have the signaller immediately suspend and hand control directly to the just-woken process, which is GUARANTEED the waited-for condition still holds exactly as left; MESA monitors (signal-and-continue) — used by essentially every real system, including Java's wait()/notify() and POSIX condition variables — have the signaller keep running, so the woken process must RE-CHECK the condition once it eventually re-acquires the monitor, since some other process may have changed it in the meantime.
3. THIS is precisely why idiomatic Mesa-style code ALWAYS wraps a condition wait in a WHILE loop (while(!condition) x.wait();) rather than a plain if — a plain if assumes the condition is still true immediately upon waking, an assumption only Hoare semantics actually guarantees.

GATE TRAP: assuming a monitor's signal() behaves like a semaphore's signal() (incrementing a count that a LATER waiter could still consume) is a direct and consequential error — a monitor's signal() on a condition variable with NO current waiters is simply LOST, discarded with no effect, unlike a semaphore's own signal, which always increments its counter regardless of whether anyone is currently waiting.
...
2. Trace Round Robin, quantum 3, on P1 (arrival 0, burst 6), P2 (arrival 2, burst 4), P3 (arrival 3, burst 3), using the convention that a process arriving exactly at a preemption instant is enqueued before the just-preempted process. Find the response time of P3.
   P1 runs first (only process ready at t=0), from 0 to 3, remaining 6−3=3. During this run, P2 arrives at t=2 and queues. At t=3, P1's quantum expires with 3 units left; P3 also arrives exactly at t=3, so it is enqueued ahead of the re-inserted P1: queue becomes [P2, P3, P1]. P2 (burst 4) runs 3 to 6, using its whole quantum but not finishing (remaining 4−3=1), so it is preempted; queue becomes [P3, P1, P2]. P3 runs 6 to 9, completing (burst exactly 3, matching the quantum). Queue: [P1, P2]. P1 runs its remaining 3 units, 9 to 12, completing. Queue: [P2]. P2 runs its last 1 unit, 12 to 13, completing.
