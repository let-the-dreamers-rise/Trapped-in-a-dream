...
A related but DISTINCT interval-greedy problem from activity selection: given a set of intervals, find the MINIMUM number of points such that every interval contains at least one chosen point ("stabbing" or "piercing" every interval) — this differs from activity selection (which selects a maximum SUBSET of non-overlapping intervals to KEEP) and from interval partitioning/min-platforms (which asks how many resources are needed to host all intervals SIMULTANEOUSLY); here, the intervals are all kept, and the question is how few points can collectively touch every one of them.

1. SORT all intervals by their RIGHT endpoint (ascending) — the same sort key used for activity selection, but exploited differently here.
2. Process intervals in this sorted order; whenever the CURRENT interval is not already stabbed by a previously placed point, place a NEW point exactly at this interval's RIGHT endpoint, and mark every other interval that also contains this point as now stabbed.
3. Continue through the sorted list, skipping any interval already stabbed by a previously placed point, and placing a new point (again at that interval's own right endpoint) whenever an unstabbed interval is reached.
4. WHY placing the point at the right endpoint (rather than anywhere else inside the interval) is optimal: the right endpoint is the LATEST possible position within the current interval, which maximises the chance that this SAME point also falls inside as many LATER (by right-endpoint order) overlapping intervals as possible — placing the point any earlier within the interval could only stab an equal or SMALLER set of the intervals that come after it in sorted order.

GATE TRAP: confusing "minimum points to stab all intervals" with activity selection's "maximum number of non-overlapping intervals to select" is easy since both sort by right endpoint — but activity selection DISCARDS overlapping intervals to keep only a compatible subset, while this problem KEEPS every interval and instead counts how many points are needed to touch all of them; the two problems can even produce numerically different answers on the identical input, so always confirm from the question's own wording which of the two is actually being asked.

1. Worked example (minimum stabbing points, independently verified): intervals [1,4], [2,6], [5,8], [7,10]. Sort by right endpoint (already sorted): [1,4], [2,6], [5,8], [7,10]. Process [1,4]: not yet stabbed, place a point at its right endpoint, 4. Check [2,6]: does it contain 4? Yes (2≤4≤6) — mark it stabbed by this same point. Check [5,8]: does it contain 4? No (4<5) — place a NEW point at [5,8]'s right endpoint, 8. Check [7,10]: does it contain 8? Yes (7≤8≤10) — mark it stabbed by this same point. Total points placed: 2 (at positions 4 and 8), stabbing all four intervals.
