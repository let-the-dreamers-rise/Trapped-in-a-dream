...
(function(){ var t = window.GATE_DATA.questions['coa'].topics.find(function(t){return t.id==='coa-io';});
  t.theory.deep += "\n\nFROM ZERO: FOUNDATIONS\n\nTerm. I/O (Input/Output). How the CPU communicates with EVERYTHING outside itself and RAM — keyboards, disks, network cards, printers. I/O devices are typically thousands to millions of times SLOWER than the CPU, so a huge part of I/O design is about NOT wasting CPU time waiting for slow devices.\n\nTerm. Programmed I/O (PIO). The CPU itself repeatedly CHECKS (\"polls\") a device's status register in a tight loop, waiting until the device signals \"ready,\" then the CPU itself copies the data one word at a time. Simple to implement, but the CPU is 100% BUSY-WAITING the whole time — completely unusable for any other work during the transfer.\n\nTerm. Interrupt-driven I/O. Instead of polling, the CPU issues a request to the device and then goes back to doing OTHER useful work; when the device finishes and is ready, it sends an INTERRUPT signal that makes the CPU briefly pause its current work, run a small handler routine to service the device (e.g. copy one word), then resume what it was doing. Much better CPU utilization than PIO, but each interrupt still costs some CPU time (context-save/restore overhead) and still requires the CPU to be involved in EVERY word transferred.\n\nTerm. DMA (Direct Memory Access). A separate, dedicated piece of hardware (the DMA CONTROLLER) that the CPU programs ONCE with a source address, destination address, and transfer length — the DMA controller then handles the ENTIRE bulk transfer directly between the device and memory, WITHOUT the CPU touching each individual word, freeing the CPU to do other work for the whole transfer's duration. The CPU is only interrupted ONCE, at the very END of the whole transfer (not once per word).\n\nKEY: the clear ranking by CPU involvement, least efficient to most: Programmed I/O (CPU fully busy the whole transfer) < Interrupt-driven I/O (CPU involved once PER WORD/UNIT, but free in between) < DMA (CPU involved only at SETUP and at the FINAL completion interrupt, essentially free during the bulk transfer itself).\n\n• Why memory-mapped vs isolated I/O matters as background: some architectures give I/O devices their OWN separate address space with special instructions (IN/OUT) to access them (isolated/port-mapped I/O); others treat device registers as if they were just ordinary memory addresses, usable with normal LOAD/STORE instructions (memory-mapped I/O) — this affects which instructions can talk to a device but is a separate question from HOW the transfer's bulk data movement is organized (PIO/interrupt/DMA).\n• Why an instruction actually \"doing something\" ties in here: every PIO word-transfer is literally a LOAD or STORE instruction executed by the CPU in a loop — this is exactly why PIO consumes normal CPU instruction-execution time proportional to the data size, while DMA's bulk movement consumes NO CPU instructions at all for the data itself.\n\nEVERY EDGE CASE\n\nGATE TRAP: whether \"DMA steals cycles\" or \"blocks the bus\" depends on the SPECIFIC DMA mode, and these are DIFFERENT things with different performance implications. CYCLE STEALING mode: the DMA controller takes control of the memory BUS for just ONE cycle at a time, in between CPU memory accesses, transferring one word per stolen cycle — the CPU is SLOWED DOWN slightly (each stolen cycle delays a CPU memory access by one cycle) but is never fully halted. BURST (BLOCK) mode: the DMA controller takes control of the bus CONTINUOUSLY for the ENTIRE transfer (many consecutive cycles) — the CPU is COMPLETELY BLOCKED from any memory access for that whole burst, a bigger single interruption but a shorter total occupation of the bus overall compared to spreading the same transfer out via cycle stealing.\n\nGATE TRAP: a GATE question asking \"how much is the CPU slowed down by DMA\" MUST specify (or you must determine from context) which mode is used — cycle stealing gives a small, spread-out slowdown proportional to transfer size (verified: transferring 1024 words with 1 cycle stolen per word costs exactly 1024 CPU cycles of delay, spread across the whole transfer), while burst mode gives ZERO slowdown to individual CPU instructions BEFORE and AFTER the burst, but a CPU instruction NEEDING the bus DURING the burst simply cannot proceed until the burst ends.\n\nGATE TRAP: interrupt SERVICING cost must be counted PER INTERRUPT, and the total overhead for transferring N units of data via interrupt-driven I/O is N * (per-interrupt overhead), which can become LARGER than DMA's single setup+completion overhead for large N — this crossover point (the N at which DMA starts winning) is a common \"which method is faster for this transfer size\" GATE calculation.\n\nGATE TRAP: DMA still needs to ACQUIRE the bus before it can transfer, and if the CPU is ALSO trying to use the bus at that moment, some ARBITRATION scheme (e.g. fixed priority, or round-robin) decides who goes first — a GATE question describing bus arbitration delays is testing a DIFFERENT (additional) overhead source layered on top of the raw transfer time itself.\n\nGATE TRAP: \"interrupt latency\" (time between a device raising an interrupt and the CPU actually starting to service it) is a DIFFERENT quantity from \"interrupt SERVICE time\" (time the handler itself takes to run) — a question giving one and asking for a total using both must add them, never conflate them, and must also account for whether interrupts are DISABLED during servicing of another interrupt (which would extend the effective latency for any interrupts arriving in the meantime).\n\nWORKED EXAMPLES\n\nWORKED EXAMPLE 1 — DMA cycle-stealing transfer time, transferring 4096 bytes as 4-byte words, CPU clock 1 GHz (1 ns per cycle), 1 cycle stolen per word transferred.\n1. Number of words = total bytes / bytes per word = 4096 / 4 = 1024 words.\n2. Cycles stolen from the CPU = 1 cycle per word transferred (cycle-stealing mode) = 1024 cycles.\n3. Real time the CPU is delayed = cycles stolen * clock period = 1024 * 1 ns = 1024 ns = 1.024 microseconds (verified by direct computation) — this is the TOTAL extra delay spread across however long the whole transfer takes, NOT a single continuous block of blocked time.\n\nWORKED EXAMPLE 2 — comparing Programmed I/O to DMA for the same 4096-byte (1024-word) transfer.\n1. Programmed I/O: assume the CPU spends 2 cycles per word (1 cycle to check the device-ready status flag, 1 cycle to actually move the word) in a busy-wait polling loop. Total CPU cycles consumed = 1024 words * 2 cycles/word = 2048 cycles = 2048 ns (verified by direct computation) — and critically, the CPU can do ABSOLUTELY NOTHING ELSE during this entire time, since it is trapped in the polling loop.\n2. DMA (cycle stealing): as computed above, only 1024 CPU cycles (1024 ns) of DELAY are imposed on the CPU, and — this is the key difference — the CPU is FREE to execute its OWN, unrelated instructions in between the stolen cycles, rather than being trapped in a dedicated loop; the \"1024 ns\" here represents slowdown of whatever the CPU was already doing, not exclusive occupation.\n3. Net comparison: DMA imposes HALF the raw cycle count of this PIO example (1024 vs 2048) AND, unlike PIO, does not force the CPU into a dedicated do-nothing-else loop — both effects favor DMA, and this gap widens further as transfer size grows, since PIO's cost scales as 2 cycles/word while DMA's cycle-stealing cost here scales as only 1 cycle/word PLUS the CPU keeps making independent progress throughout.\n\nWORKED EXAMPLE 3 — finding the break-even transfer size between interrupt-driven I/O and DMA.\n1. Suppose interrupt-driven I/O costs a fixed overhead of 50 cycles PER WORD transferred (context save/restore plus the actual word move inside the handler), while DMA costs a fixed SETUP overhead of 200 cycles ONCE, plus 1 cycle PER WORD (cycle stealing) for the bulk transfer, plus a fixed 100-cycle COMPLETION-interrupt overhead at the very end.\n2. Interrupt-driven total for N words = 50*N cycles. DMA total for N words = 200 + 1*N + 100 = 300 + N cycles.\n3. Set them equal to find the break-even N: 50N = 300 + N -> 49N = 300 -> N = 300/49 ≈ 6.12 words (verified arithmetically).\n4. Interpretation: for any transfer of about 7 words or MORE, DMA's fixed setup cost is already repaid and DMA becomes cheaper; for a TINY transfer of just 1-6 words, the per-word overhead difference has not yet outweighed DMA's fixed 300-cycle setup+completion cost, so interrupt-driven I/O (or even simple PIO) can actually be cheaper for very small transfers — this is exactly why real systems often use PIO/interrupts for small, latency-sensitive transfers and reserve DMA for large, bulk transfers."; })();
